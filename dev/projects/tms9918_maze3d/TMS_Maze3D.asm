...
mob_max_hp: .res 1
combat_msg: .res 1     ; line index in combat log
ev_dmg:     .res 1     ; $31
...
; =============================================
; debug_mark: write a single 'H' at cell (0,0) to confirm we got here
; =============================================
debug_mark:
        LDA #0
        STA ch_cx
...
; =============================================
; key_or_zero_blocking: same as wait_key but non-blocking.
; Returns A=key if available, A=0 otherwise.  Also stirs prng.
; =============================================
poll_key:
        INC prng_lo
        BNE @nh
...
; =============================================
; key_or_zero: non-blocking key probe (A=key or 0)
; =============================================
key_or_zero:
        LDA KBDCR
        BPL @no
...
@step:
        ...
        CMP ln_y1
        BEQ @end
@do:    ; e2 = err*2  -> compare 2*err vs +/-dy and dx
        LDA ln_err
        ASL                     ; 2*err (signed); we'll detect sign with BPL/BMI on original err
        STA tmp                 ; 2*err for x direction test
        ; if 2*err > -dy:  err -= dy; x0 += sx
        ; equivalent: if err > -dy/2 ... but easier: compare ln_err to -dy/2? Use original logic:
        ; Bresenham: e2 = 2*err; if e2 > -dy then err -= dy; x0 += sx; if e2 <  dx then err += dx; y0 += sy
        ; We need -dy comparison. Compute (-dy) as 2's complement.
        SEC
        LDA #0
        SBC ln_dy               ; A = -dy (signed in 8 bits if dy<128)
        STA tmp2                ; -dy
        ; signed compare: tmp (2*err) > tmp2 (-dy)?
        LDA tmp
        SEC
        SBC tmp2
        BVC @s1                 ; no overflow
...
; =============================================
; hline: horizontal line from (fl_x0,fl_y0) to (fl_x1,fl_y0)
; pixel by pixel via plot_set (slow but simple)
; =============================================
hline:
        LDA fl_x0
        STA pix_x
...
; =============================================
; vline: vertical line at fl_x0 from y=fl_y0 to fl_y1
; =============================================
vline:
        LDA fl_x0
        STA pix_x
...
; =============================================
; fill_rect: fills rectangle (fl_x0..fl_x1, fl_y0..fl_y1) using
; pattern selected by fl_pat (0..3):
;   0 = solid    $FF every pixel
;   1 = bricks   alternating row of $FF / $00 modulated by 4-wide vbar
;   2 = stipple  $AA on even row, $55 on odd
;   3 = hatch    $11 / $44 sparse
; The fill is row-by-row using plot_set with a per-row pattern byte.
; To stay simple we treat the rect bit-by-bit.
; =============================================
fill_rect:
        LDA fl_y0
        STA pix_y
@yrow:
        ...
        STA pix_x
@xcol:
        ; decide pixel state based on (x,y) and pattern
        LDX fl_pat
        LDA pix_x
        STA tmp
        LDA pix_y
        STA tmp2
        JSR pattern_pixel       ; A = 0 keep / non-zero plot
        BEQ @skip
        JSR plot_set
@skip:
        LDA pix_x
...
; =============================================
; pattern_pixel: returns A!=0 if (tmp,tmp2) belongs to fl_pat
; X = pattern id
;   0 solid           always 1
;   1 bricks          y%4==0 line OR (x+y)%8==0 column tick
;   2 stipple checker (x ^ y) & 1
;   3 hatch           (x+y) & 3 == 0
;   4 dense-stipple   (x ^ y) & 1 ^ ... -> simply 1 every 2 pixels diagonally
;   5 v-bars          x & 3 == 0
; =============================================
pattern_pixel:
        TXA
...
@p1:    CPX #1
        BNE @p2
        ; bricks: y%4==0 OR x%6==0
        LDA tmp2
        AND #$03
        BEQ @on
        LDA tmp
        ; x%6 = x - 6*(x/6); approximate with x%4==0 for speed
        AND #$03
        BEQ @on
        LDA #0
        RTS
...
; =============================================
; clear bitmap full -> just JSR clear_bitmap
; clear single 8-pixel cell row band: clear_band_y0_y1 (fl_y0..fl_y1)
; (used for combat log lines, etc.) - implemented via fill_rect with pat=0 on 0..255
; =============================================
clear_band:
        LDA #$00
        STA fl_x0
        ...
        STA fl_x1
        LDA #$00
        STA fl_pat              ; pattern 0 = solid; we want to ERASE
        ; Specialized erase: write zero bytes per cell row
        ; addr: walk bytes for (y between fl_y0 and fl_y1)
        LDA fl_y0
        STA pix_y
@yrow:
        LDA #0
        STA pix_x
@xcol:
        ; clear the pixel: read-modify-write zero bit. Easier: write zero byte per 8 cols.
        LDA pix_x
        AND #$07
        BNE @bumpx
...
; =============================================
; fill_left_trap: diagonal hatching inside the left wall trapezoid.
; Trapezoid corners:
;   A=(near_x, near_ty)   B=(far_x, far_ty)
;   D=(near_x, near_by)   C=(far_x, far_by)
; We draw 3 diagonal hatch lines from the top edge AB to the
; bottom edge DC, evenly spaced in x. That gives the impression
; of stone seams without doing any pixel-by-pixel arithmetic.
; =============================================
fill_left_trap:
        LDX rd_depth
        LDA frame_lx,X          ; near x
        STA tmp                 ; tmp = near_x
        LDA frame_lx+1,X        ; far x
        STA tmp2                ; tmp2 = far_x
        ; mid_x = (near_x + far_x) / 2
        CLC
        ADC tmp
        ROR
        STA fl_x0               ; fl_x0 = mid_x

        ; quarter points: q1 = (near + mid)/2, q3 = (mid + far)/2
        LDA tmp                 ; near
        CLC
        ADC fl_x0
        ROR
        STA fl_x1               ; q1 = near..mid midpoint
        LDA fl_x0
        CLC
        ADC tmp2
        ROR
        STA fl_y0               ; q3 = mid..far midpoint (reuse fl_y0)

        ; Hatch line 1: from (q1, top_at_q1) to (q1, bot_at_q1)
        ; Approximation: use a vertical column at q1 with y from
        ; near_ty + 1 to near_by - 1 — close enough for a 8 px gap.
        ; (Visual hint of brick verticals.)
        LDX rd_depth
        LDA frame_ty,X
        CLC
        ADC #2
...
@h2:    JSR plot_set
        ...
        CMP frame_by,X
        BCC @h2
        ; horizontal mid-line (mortar) along the wall
        LDX rd_depth
        LDA frame_lx,X
        CLC
        ADC #1
        ...
        CLC
        LDY frame_by,X
        STY tmp                 ; not used; we approximate mid_y
        ; mid_y = (ty + by) / 2
        ADC frame_by,X
        ROR
        STA pix_y
...
; =============================================
; fill_right_trap: mirror — diagonal hatching for right wall.
; =============================================
fill_right_trap:
        LDX rd_depth
        LDA frame_rx,X          ; near x (larger)
        STA tmp
        LDA frame_rx+1,X        ; far x (smaller)
        STA tmp2
        ; mid_x = (near + far) / 2
        CLC
        ADC tmp
...
; =============================================
; fill_front_block: brick hatching inside the closing front wall.
; Two horizontal seams + a vertical seam in the middle.
; =============================================
fill_front_block:
        LDX rd_depth
        LDA frame_lx,X
...
; =============================================
; draw_mob_indicator: if the next forward cell has a mob, show '!'
; =============================================
draw_mob_indicator:
        ; compute target = (p_col + dx, p_row + dy)
        LDA p_col
        CLC
        ADC rd_dx
        STA tmp
...
write_char_at_xy_pixels_S:
        ; placeholder for writing 'S' at fixed top-left cell
        LDA #6
        STA ch_cx
        LDA #4
        STA ch_cy
