#!/usr/bin/env python3
"""Construit GEN2DBuf (C / GEN2 HGR, double buffering) avec cl65 et émet son
sidecar Woz-hex.
...
Sorties sous "software/Graphic HGR/":
    GEN2DBuf.bin   — binaire brut, origine $6000
    GEN2DBuf.txt   — Wozmon hex (AAAA: BB BB ...) + une ligne 6000R finale

Run:  python3 emit_GEN2DBuf_txt.py   (le Makefile l'appelle après le build)
"""
import pathlib
import subprocess
import sys
...
def main() -> int:
    ...
    out_bin = OUT_DIR / "GEN2DBuf.bin"
    out_txt = OUT_DIR / "GEN2DBuf.txt"
