#!/usr/bin/env python3
"""Build GEN2Countdown (C / GEN2 HGR, plain 20→0 counter) and emit Woz-hex.
...
def main() -> int:
    ...
    emit_cl65(
        c_files=["GEN2Countdown.c"],
        extra_sources=[
            # Trimmed to the families this demo calls. ld65 links explicit
            # objects in FULL (only library members are dead-stripped), so
            # listing unused families wastes ROM/RAM. Here: CORE + pixel
            # (unplot) + rect (fill_rect) + text (puts/putu) + geom (circle).
            # Dropped: sprites, lores.
            f"{GEN2C}/gen2_init.c",
