...
2. **`BIT $CC01` (ou `LDA $CC01`) entre chaque écriture.** Macro `TMS_IO_DELAY()` côté nippur72 (`apple1-videocard-lib/lib/utils.h:14`, dépôt externe nippur72 — non vendoré dans ce repo). Coût : 4 cycles CPU + un accès bus qui passe par la machine à slots du VDP, donc « cale » la boucle sur la cadence du chip. Effet collatéral : **efface F (bit 7), 5S (bit 6) et C (bit 5), et reset le flip-flop** (datasheet TI §2.2 / Nouspikel / openMSX — re-corrigé juillet 2026 ; seuls les bits d'index 0..4 survivent ; cf. *Bug N°9* dans `Programming_TMS9918.md` §19 et §13). À éviter quand le code surveille le statut, sûr partout ailleurs.

3. **Burst chunké en VBLANK.** Pattern complet : `tms_wait_end_of_frame()` puis ≤ 128 octets dans une boucle serrée, recommencer à la trame suivante au besoin. Référence : `apple1-videocard-lib/lib/screen1.c:47-90` (scroll vertical, dépôt externe nippur72 — non vendoré dans ce repo), commentaire explicite sur la fenêtre strict-mode (~4 500 cycles de slots `slotsMsx1ScreenOff` disponibles par trame). Adapté aux gros transferts (scroll, refresh complet d’écran).
...
- **Divergence par défaut (rectifié)** : contrairement à ce qu’affirmait cette section, les échecs ne sont **pas** partagés dans la configuration **par défaut**. POM1 démarre la VRAM en motif bistable `$FF/$00` (`vramNoiseOnReset` OFF par défaut) → les octets Y non initialisés de la SAT valent `$FF` (hors écran), donc un jeu qui n’écrit que `SAT[0].Y=$D0` et **omet** le fill `SAT[1..127]=$D1` rend **correctement sous POM1 mais affiche des sprites parasites sur le vrai TMS9918A**. La vue « échecs partagés » ne vaut que lorsque le **bruit VRAM est armé** (`--vram-noise`, et désormais **par défaut sur les presets silicium**). Rapport 2026‑06‑29 : **cassés sur silicium** Galaga, Rogue, **Snake**, **Sokoban**, Mandelbrot ; **OK** Logo (fix `4d73a42`), Plasma, Life. Les correctifs visent donc les sources cc65 / ASM (fill défensif `SAT[1..127]=$D1` cf. §4.2 ; init des données §9) **et** le défaut émulateur qui doit armer le bruit VRAM sur les presets silicium pour **exposer** la divergence (sinon POM1 la masque). Voir `TODO.md` › Fidélité TMS9918.
- **/INT câblé mais jamais asservi** : à l’oscilloscope la ligne reste à +5 V solide pendant Tetris, Plasma, Galaga — non parce qu’elle est déconnectée (Parmigiani a vérifié le câblage /INT → /IRQ), mais parce que ces programmes pollent `$CC01` sans jamais démasquer l’IRQ (`CLI`), donc /INT n’est jamais tiré bas durablement. Cohérent avec § 8 (pattern de polling `LDA $CC01 / BPL`) et `Programming_TMS9918.md` §18 (Bug N°2).
...
Pour le détail des bugs silicon et tests de régression dans POM1, voir aussi `Programming_TMS9918.md` et les tests `tms9918_*` listés dans `CLAUDE.md`.
