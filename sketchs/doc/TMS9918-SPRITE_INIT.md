...
2. **`BIT $CC01` (ou `LDA $CC01`) entre chaque écriture.** Macro `TMS_IO_DELAY()` côté nippur72 (`dev/apple1-videocard-lib/lib/utils.h:14`). Coût : 4 cycles CPU + un accès bus qui passe par la machine à slots du VDP, donc « cale » la boucle sur la cadence du chip. Effet collatéral : **efface F/5S/C et reset le flip-flop** (cf. *Bug N°9* dans `Programming_TMS9918.md` §19). À éviter quand le code surveille le statut, sûr partout ailleurs.
