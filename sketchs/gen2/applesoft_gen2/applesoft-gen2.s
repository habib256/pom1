; Applesoft GEN2
;
; Applesoft Lite (Microsoft 6502 BASIC) + Uncle Bernie's GEN2 colour graphics.
; Base = txgx42/applesoft-lite; CFFA1 disk I/O removed and replaced with the
; GEN2 TXT/LGR/HGR primitives (soft switches $C250-$C257). The three freed CFFA
; statement tokens (MENU/SAVE/LOAD) become TEXT/GR/HGR; COLOR=/HCOLOR=/PLOT/
; HLIN/VLIN/HPLOT are added as six new statement tokens ($A2-$A7), which shifts
; every operator + function token up by 6 (TOKEN_* equates updated below). The
; graphics handlers + lookup tables live in gen2gfx.inc (.include'd at EOF).
;
; Disassembled from the Apple II+ ROMs with da65 V2.12.0
; Most comments and label names from the S-C DocuMentor by Bob Sander-Cederlof
; Applesoft Lite adapted for the Replica-1 by Tom Greene (7-May-2008)
;
; v0.4 + GEN2
...
.setcpu "6502"
; POM1/DevBench: modern ca65 (>= 2.18) rejects the negative immediates this
; 2008 source relies on (e.g. `lda #-9`, `ldx #-4`) and parses `#<INPUTBUFFER-1`
; as `(<INPUTBUFFER)-1`. force_range silently truncates such expressions to the
; intended low byte, reproducing the original ROM byte-for-byte. (cc65 had this
; as the default behaviour when Applesoft Lite was first assembled.)
.feature force_range
.segment "BASIC"
...
; (CFFA1 disk I/O removed for the GEN2 build — graphics handlers are local.)
...
TOKEN_ADDRESS_TABLE:
	...
	; Order: HGR2/GR2 precede HGR/GR so the prefix-greedy tokenizer matches
	; the longer keyword first (else "HGR2" would tokenize as HGR + "2").
	.addr	GFX_HGR2 - 1	; $9E... HGR2  (hi-res page 2)
	.addr	GFX_HGR - 1	; $9F... HGR
	.addr	GFX_GR2 - 1	; $A0... GR2   (lo-res page 2)
	.addr	GFX_GR - 1	; $A1... GR
	.addr	GFX_TEXT - 1	; $A2... TEXT
	.addr	CLS - 1		; $A3... CLS
	.addr	GFX_COLOR - 1	; $A4... COLOR=
	.addr	GFX_HCOLOR - 1	; $A5... HCOLOR=
	.addr	GFX_PLOT - 1	; $A6... PLOT
	.addr	GFX_HLIN - 1	; $A7... HLIN
	.addr	GFX_VLIN - 1	; $A8... VLIN
	.addr	GFX_HPLOT - 1	; $A9... HPLOT
	.addr	GFX_SHOW - 1	; $AA... SHOW n (flip displayed page)
	.addr	GFX_VBL - 1	; $AB... VBL   (wait vertical blank)
	.addr	GFX_HOME - 1	; $AC... HOME  (clear GEN2 text page)
	.addr	GFX_HTAB - 1	; $AD... HTAB c
	.addr	GFX_VTAB - 1	; $AE... VTAB r
...
UNFNC:  .addr	SGN		; $C7... SGN
	...
	.addr	RIGHTSTR	; $D0... RIGHT$
	.addr	MIDSTR		; $D1... MID$
	.addr	SCRN_PARSE	; $D2... SCRN(x,y) (intercepted in UNARY; entry unused)
...
TOKEN_NAME_TABLE:
	...
	htasc	"GET"		; $9C... 156
	htasc	"NEW"		; $9D... 157
	; HGR2/GR2 before HGR/GR (prefix-greedy tokenizer — see address table).
	htasc	"HGR2"		; $9E... GEN2: hi-res page 2
	htasc	"HGR"		; $9F... GEN2: hi-res mode
	htasc	"GR2"		; $A0... GEN2: lo-res page 2
	htasc	"GR"		; $A1... GEN2: lo-res mode
	htasc	"TEXT"		; $A2... GEN2: text mode
	htasc	"CLS"		; $A3... clear Apple-1 text screen
	htasc	"COLOR="	; $A4... GEN2: lo-res colour
	htasc	"HCOLOR="	; $A5... GEN2: hi-res colour
	htasc	"PLOT"		; $A6... GEN2: lo-res point
	htasc	"HLIN"		; $A7... GEN2: lo-res horizontal line
	htasc	"VLIN"		; $A8... GEN2: lo-res vertical line
	htasc	"HPLOT"		; $A9... GEN2: hi-res point / line
	htasc	"SHOW"		; $AA... GEN2: display page 1|2
	htasc	"VBL"		; $AB... GEN2: wait vertical blank
	htasc	"HOME"		; $AC... GEN2: clear text page + home cursor
	htasc	"HTAB"		; $AD... GEN2: cursor column
	htasc	"VTAB"		; $AE... GEN2: cursor row
...
; "PRINT" STATEMENT
; ----------------------------------------------------------------------------
; GEN2: entered via the PRINT (CSW=GCOUT) and APRINT (CSW=MONECHO) wrappers in
; gen2gfx.inc, which set the output vector then jsr here. PRINTCORE is the stock
; Applesoft PRINT logic, unchanged, ending in rts.
PRINTCORE:
...
; ----------------------------------------------------------------------------
UNARY:	cmp	#TOKEN_SCRN	; GEN2 SCRN(x,y): 2-arg numeric, own parser
	bne	@nf
	jmp	SCRN_PARSE
@nf:	asl	a		; DOUBLE TOKEN TO GET INDEX
	pha
	tax
	jsr	CHRGET
...
COLDSTART:
	...
	jsr	GEN2_BOOT	; GEN2: init output vector (CSW), draw pages,
				; cursor, default colours, text mode + clear
...
; ----------------------------------------------------------------------------
	; GEN2: HIMEM is FIXED at $9800 (just below this interpreter image)
	; instead of probing RAM upward — on the 48 KB GEN2 machine the probe
	; would walk past $BFFF / into the I/O space. BASIC program + variables
	; + strings therefore own $0801-$97FF (~37 KB). The HGR framebuffer at
	; $2000-$3FFF lives inside that window, so an HGR program must keep its
	; program text + string space clear of $2000-$3FFF (real-silicon-faithful
	; tradeoff: maximal BASIC room rather than a framebuffer-safe 6 KB cap).
	lda	#>$9800		; HIMEM hi byte
	ldy	#<$9800		; HIMEM lo byte ($00)
	sty	MEMSIZ
	sta	MEMSIZ+1
...
; ----------------------------------------------------------------------------
; GEN2 graphics command handlers + Uncle Bernie soft-switch / lookup tables.
; Shares this file's namespace (GETBYT, COMBYTE, GETADR, CHKCOM, SYNCHR, ...),
; so it is .include'd here rather than linked as a separate module.
; ----------------------------------------------------------------------------
	.include "gen2gfx.inc"
