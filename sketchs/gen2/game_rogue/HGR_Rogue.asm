...
.globalzp ht_cm_ev, ht_cm_od, ht_cbit, ht_page, bl_page
...
start:
        ...
        LDA #0
        STA ht_cbit
        STA ht_page             ; single-buffered: page 1 everywhere
        STA bl_page
        LDA #1
...
; ----------------------------------------------------------------------------
; render_tiles: repaint the DIRTY cells of the 16x10 playfield as
; 14x16 px HGR tiles. Two bit-packed shift registers walk each row in
; column order (same trick as the TMS render_map):
;   cur_pak  = vis_buffer row bytes            -> "is the cell lit NOW"
;   vis_pak  = vis_buffer | prev_vis row bytes -> "does it need a redraw"
; A dirty-but-dark cell repaints as TILE_EMPTY (all-zero bitmap), which
; is what re-blanks cells that just left the FOV and erases entity
; ghosts (entities only ever render on lit cells). Hidden pits render
; blank exactly like the TMS version. Ends by snapshotting vis_buffer
; into prev_vis for the next frame's dirty set.
; ----------------------------------------------------------------------------
render_tiles:
        LDA     #0
        STA     rt_row
@row_lp:
        LDA     rt_row
        ASL                             ; Y = row * 2 (vis bytes per row)
        TAY
        LDA     vis_buffer,Y
        STA     cur_pak_lo
        EOR     prev_vis,Y              ; FOV ring delta…
        ORA     ent_prev,Y              ; …plus every cell an entity
        ORA     ent_now,Y               ; touched last frame or this one
        STA     vis_pak_lo
        LDA     vis_buffer+1,Y
        STA     cur_pak_hi
        EOR     prev_vis+1,Y
...
@col_lp:
        LSR     cur_pak_hi
        ROR     cur_pak_lo              ; C = cell lit now
        LDA     #0
        ROL                             ; A = 0/1 live bit
        STA     rt_lit
        LSR     vis_pak_hi
        ROR     vis_pak_lo              ; C = dirty (lit now or last frame)
        BCS     @dirty
        JMP     @next
@dirty:
        LDA     rt_lit
        ...
        JMP     @draw
@lit:
        ; Fetch the packed nibble: byte = row*8 + col/2.
        LDA     rt_col
        LSR
        STA     tmp_packed
        LDA     rt_row
...
@have_nib:
        ...
        LDA     tmp_packed
        AND     #TILE_REVEAL_BIT
        BNE     @show_pit               ; revealed pit -> pictogram
...
@m_col:
        ...
        JSR     blit_sprite_a           ; (blit engine never touches X)
        JMP     @mon_skip
@boss_paint:
...
@b_col:
        ...
        LDA     #32
        STA     bl_h
        JSR     hgr_blit4               ; 28x32 px, 2x2-cell footprint
@mon_skip:
        TXA
        CLC
        ADC     #MON_SIZE
...
@item_done:
        ...
        JSR     blit_sprite_a           ; its cell is in ent_now (pre-scan)
@done:
        RTS
