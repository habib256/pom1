; =============================================
; HGR_ROGUE X2 - the x2 COLOUR variant of the GEN2 HGR port.
; Same game logic; the playfield renders at x2 (28x32 px cells, coloured
; assets with NTSC parity masks baked in) through an 8x5-cell viewport
; CENTRE-LOCKED on the player (the hero pins viewport cell (3,2), the
; world scrolls around them; off-map cells render black) — the 16x10 map
; at x2 no longer fits the 280-px screen. Hits flash by flipping the
; NTSC palette bit (hgr_blit2 PALFLIP: green<->orange, violet<->blue).
; The 4 HUD text rows keep the x1 geometry and icons, colour-accented.
; DOUBLE-BUFFERED: every turn renders to the hidden HGR page and flips
; ($C254/$C255) — no visible repaint at all. The dirty snapshots are
; kept PER PAGE (swapped at flip time so render_tiles' delta formula is
; untouched); the HUD repaints once per page via hud_again; modals and
; prompts draw straight onto the visible page (draw_to_front).
; Derived from sketchs/gen2/game_rogue/HGR_Rogue.asm.
; =============================================
; HGR_ROGUE - GEN2 HGR Color Graphics Card port
; VERHILLE Arnaud - 2026
; Roguelike for the Apple-1 + Uncle Bernie's GEN2 HGR card. Port of
; sketchs/tms9918/game_rogue (TMS_Rogue.asm): the game logic (dungeon
; gen, FOV shadowcasting, monsters, combat, inventory, boss) is carried
; over verbatim; the whole video layer is rewritten for the 280x192
; HGR bitmap (no name table, no hardware sprites).
; =============================================
; Display: HGR page 1 ($2000-$3FFF), 40 bytes/scanline, 7 px/byte
; (bit 0 = leftmost, bit 7 = palette group, kept clear).
; The TMS 32x24 char grid maps 1:1 onto a 32-wide byte-column window:
;   text col C (0..31)  -> HGR byte column 4 + C   (8x8 glyph, byte-aligned)
;   text row R (0..23)  -> scanline R * 8
;   logical cell (L,R)  -> byte col 4 + 2L, scanline R*16 (14x16 px tile)
; Playfield rows 0..19 (scanlines 0..159) = 16x10 logical cells;
; HUD rows 20..23 (scanlines 160..191). Entities (player / monsters /
; items / boss / thrown dagger) are soft-blitted OVER the tile layer on
; every repaint; the TMS per-sprite "hurt" red flash becomes an
; inverted-box blit (HGR has no per-sprite colour).
;
; Repaints are dirty-tracked: render_tiles redraws only cells lit this
; frame OR last frame (prev_vis snapshot). Entities only ever render on
; lit cells, so their ghosts are covered by the dirty set; the thrown
; dagger (which may fly into darkness) dirties its own cell, and
; fresh-level / modal-exit paths force a full repaint.
; =============================================

.include "apple1.inc"
.include "gen2.inc"          ; GEN2 soft-switch equates (READ-only!)

; --- TMS9918 compatibility layer --------------------------------------------
; The game logic below was written against the TMS9918 lib helpers.
; Keeping their names lets ~5000 lines of logic assemble untouched:
;   WRT_DATA_REG / WRT_DATA_VAL -> hgr_putc8 (dev/lib/gen2/hgr_text8.asm):
;                                  8x8 glyph at the text cursor, auto-
;                                  advancing + row-wrapping exactly like
;                                  the VDP write pointer.
;   name_at_rc / vdp_set_write  -> text-cursor math on (vdp_row, vdp_col)
;                                  / a $1800-based name-table address.
;   WAIT_VBLANK_SAFE / pad18    -> no-ops (no VDP bus timing here).
;   clear_name_table            -> clear_hgr (8 KB framebuffer wipe).
;   disable_sprites             -> no-op (entities are part of the repaint).
...
; --- HGR port: video-layer ZP -------------------------------------------
; vdp_* keep their TMS names — the logic stores string pointers into
; vdp_src_lo/hi and row/col into vdp_row/vdp_col all over the file.
vdp_lo:         .res 1          ; name-table addr scratch (name_at_rc out)
vdp_hi:         .res 1
vdp_src_lo:     .res 1          ; string source pointer (draw_text etc.)
vdp_src_hi:     .res 1
vdp_row:        .res 1          ; text row/col inputs for name_at_rc
vdp_col:        .res 1
ptr_lo:         .res 1          ; HGR scanline pointer (clear_hgr, gen2 lib)
ptr_hi:         .res 1
et_t:           .res 1          ; 16-bit *8 scratch (cursor + blit setup)
et_t2:          .res 1
hud_msg:        .res 1          ; a transient prompt sits on rows 22-23
hud_force:      .res 1          ; full-screen wipe took the HUD with it
cam_col:        .res 1          ; x2 viewport: top-left map cell shown
cam_row:        .res 1
camp_col:       .res 1          ; camera as of the last repaint — a move
camp_row:       .res 1          ;   shifts every pixel -> force_dirty_all
vp_mc:          .res 1          ; render loop: current map col / row
vp_mr:          .res 1
vp_force:       .res 1          ; camera moved / full wipe: repaint the
                                ;   off-map border cells on the next TWO
                                ;   renders (one per page)
shown_page:     .res 1          ; page selector currently displayed
                                ;   ($00 = HGR page 1, $60 = page 2)
hud_again:      .res 1          ; HUD painted on one page — repeat the
                                ;   same paint on the other one
tx2_col:        .res 1          ; x2 title text: dest byte column
tx2_sl:         .res 1          ; x2 title text: top scanline
tx2_row:        .res 1          ; x2 title text: source glyph row
tx2_l:          .res 1          ; x2 title text: doubled row bytes
tx2_r:          .res 1
; The text cursor (ht_*) and the blit engine ZP (bl_*) are owned by
; dev/lib/gen2/hgr_text8.asm and hgr_blit2.asm, included at the bottom.
; Forward ZP references (the modules land at EOF): tell ca65 they live
; in the zero page so every access gets short addressing.
.globalzp ht_col, ht_sl, ht_left, ht_wrap, ht_font_lo, ht_font_hi, ht_rev
.globalzp ht_cm_ev, ht_cm_od, ht_cbit
.globalzp bl_src, bl_col, bl_sl, bl_h, bl_mode
rt_row:         .res 1          ; render_tiles cell cursor
rt_col:         .res 1
rt_lit:         .res 1          ; current cell's live vis bit
rt_tile:        .res 1          ; tile id cache across set_cell_pos
cur_pak_lo:     .res 1          ; render_tiles: live-vis shift register
cur_pak_hi:     .res 1          ;   (vis_pak_lo/hi hold the DIRTY register)
...
; new_level.
vis_buffer:     .res 20
; HGR port: snapshot of the vis_buffer as of the LAST render_tiles pass.
; A cell is redrawn iff (vis | prev_vis) has its bit set — cells that
; just went dark get re-blanked exactly once, cells dark on both frames
; are skipped. clear_vis_buffer fills this with $FF (fresh level = full
; repaint); force_dirty_all does the same on modal exits.
prev_vis:       .res 20
...
;
;   monsters  $0300-$037F  16 slots × 8 B = 128 B
;   items     $0380-$039F   8 slots × 4 B =  32 B
;   inventory $03A0-$046F  26 slots × 8 B = 208 B (a..z, INV_SIZE each)
; (HGR port: the GEN2 "HGR Color" machine is 48 KB flat $0000-$BFFF —
; there is no $E000 high bank. The pools land right after MAPSEG in low
; RAM, comfortably below the $2000 framebuffer.)
monsters  = $0300
items     = $0380
inventory = $03A0
; --- HGR port: repaint-elision state (free RAM after the bag) -----------
;   hud_cache  11 B — last-painted DEPTH/HP/HPMAX/ATK/DEF/XP/DAGGER +
;              the 4 buff timers; update_hud skips its ~130-glyph repaint
;              when nothing changed (each glyph is ~350 cycles on HGR).
;   ent_prev   20 B — bit-packed cells that carried an entity LAST frame.
;   ent_now    20 B — same, THIS frame (pre-scan in place_all_sprites).
; render_tiles redraws (vis XOR prev_vis) | ent_prev | ent_now — the FOV
; ring delta plus entity cells, instead of the whole lit union.
hud_cache = $0470
ent_prev  = $0480
ent_now   = $0494
; Double buffering: the OTHER page's dirty snapshots. page_flip swaps
; these with prev_vis / ent_prev so the render code always sees "this
; page's last frame" under the usual names.
prev_vis_alt = $04B0
ent_prev_alt = $04C4
...
start:
        ...
        ; GEN2 init: park the display on TEXT, wipe HGR page 1 (the
        ; SRAM powers up as junk and the card has no display-enable
        ; bit), then flip GRAPHICS + HIRES + PAGE1 + MIXOFF.
        JSR gen2_hgr_init_clear
        ; hgr_text8 setup: bbfont (HGR bit order), the 32-col TMS text
        ; window at byte columns 4..35 with VDP-style wrap.
        LDA #<HGR_Font5F
        STA ht_font_lo
        LDA #>HGR_Font5F
        STA ht_font_hi
        ...
        LDA #36
        STA ht_wrap
        LDA #HUE_WHITE          ; hgr_text8 colour attrs: pass-through
        JSR text_color_a
        ; Double buffering: zero HGR page 2 (power-on junk) and start
        ; with page 1 displayed + drawn (the title is a front-page
        ; screen; the first dungeon render goes to the back page).
        LDA #$40
        STA ptr_hi
        LDA #0
        STA ptr_lo
...
@zp2:   STA (ptr_lo),Y
        ...
        STA hud_again
        LDA #1
        STA hud_force           ; first update_hud must paint (BSS is
        LDA #0                  ; garbage on a real cold boot)
        STA hud_msg
        STA cam_col
        STA cam_row
        LDA #$FF
        STA camp_col            ; != any real camera -> the first
        STA camp_row            ; update_camera forces a full paint
...
@std_first_level:
        ...
        JSR page_flip           ; present the first frame
...
@repaint:
        ...
        JSR place_all_sprites
        JSR update_hud
        JMP page_flip           ; present the new floor
...
; ----------------------------------------------------------------------------
; TMS9918 compatibility stubs. The pad18 JSRs sprinkled through the
; logic (silicon-strict VDP bus cushions) and the SAT disable become
; plain RTS — harmless cycle burns on a framebuffer card.
; ----------------------------------------------------------------------------
tms9918_pad18:
disable_sprites:
        RTS

; clear_name_table: TMS "wipe the 32x24 screen" == wipe the whole HGR
; framebuffer here (playfield + HUD). clear_hgr is the gen2 lib module
; included at the bottom of this file; it uses our ptr_lo/ptr_hi ZP.
clear_name_table:
        LDA     #1              ; the wipe just took rows 20-23 with it —
        STA     hud_force       ; the next update_hud may not skip
        ; page-aware clear: wipe the page bl_page/ht_page point at
        ; ($20xx or $40xx — base high byte = $20 EOR page selector).
        LDA     #$20
        EOR     ht_page
        STA     ptr_hi
        LDA     #0
...
; ----------------------------------------------------------------------------
; name_at_rc: (vdp_row, vdp_col) -> vdp_lo/hi, a $1800-based name-table
; address WITH the $40 write bit in the high byte — byte-compatible with
; the TMS lib so the callers that do `LDA vdp_lo / LDX vdp_hi /
; JSR draw_text` keep working against the hardcoded $58/$59/$5A tables.
; ----------------------------------------------------------------------------
name_at_rc:
        LDA     vdp_row
        LSR
...
; ----------------------------------------------------------------------------
; vdp_set_write / set_cursor_ax: decode a name-table address into the
; HGR text cursor (ht_col = byte column 4 + col, ht_sl = row * 8).
;   vdp_set_write : address in vdp_lo / vdp_hi
;   set_cursor_ax : address in A (lo) / X (hi, $40 bit tolerated)
; addr - $1800 = row*32 + col  ->  row = (hi&3)*8 + lo>>5, col = lo&31.
;
; vdp_set_write MUST PRESERVE X, like the TMS lib version (A-only):
; draw_inv_line keeps its inventory slot offset in X across the
; name_at_rc + vdp_set_write pair — an earlier `LDX vdp_hi` here
; leaked $58 into X and every bag line rendered as slot "[L]" with
; garbage qty/name/utility (the HGR-port inventory bug).
; ----------------------------------------------------------------------------
vdp_set_write:
        LDA     vdp_hi
        AND     #$03                    ; $58/$59/$5A -> 0..2
        ...
        AND     #$1F
        CLC
        ADC     #4                      ; text col 0 -> byte column 4
        STA     ht_col
        RTS
...
; ============================================================================
; HGR blit engine — the raw blits (hgr_blit2 / hgr_blit4, OR / flash /
; STORE via bl_mode) live in dev/lib/gen2/hgr_blit2.asm; below are the
; game-side wrappers that know the asset tables and the cell grid.
; None of these touch X — the entity loops keep pool offsets there.
; ============================================================================

; set_cell_pos: A = logical col, Y = logical row  ->  bl_col/bl_sl.
set_cell_pos:
        ASL                             ; col * 2
        CLC
        ADC     #4                      ; playfield left margin (byte 4)
        STA     bl_col
        TYA
        ASL
        ASL
...
; blit_tile_a: draw dense tile id A (0..7) at bl_col/bl_sl in
; STORE mode. The HGR tile bitmap for id X sits at rogue_tiles_hgr +
; tile_char_base[X] * 8 (CHAR_* bases 0,4..28 -> byte offsets 0,32..224).
blit_tile_a:
        TAY
        LDA     tile_char_base,Y
        ASL
        ASL
        ASL                             ; base * 8 = tile byte offset
        CLC
        ADC     #<rogue_tiles_hgr
        STA     bl_src
        LDA     #>rogue_tiles_hgr
        ...
        LDA     #16
        STA     bl_h
        LDA     #2                      ; STORE — tiles repaint their cell
        STA     bl_mode
        JMP     hgr_blit2

; blit_sprite_a: draw the 14x16 sprite whose TMS slot number is in A
; (0, 4, 8 .. 52) at bl_col/bl_sl. bl_mode is the CALLER's (0 =
; OR over the tile, 1 = inverted hurt flash). Bitmap at
; rogue_sprites_hgr + slot * 8 (16-bit offset — slot 52 * 8 = 416).
blit_sprite_a:
        STA     et_t
        LDA     #0
        STA     et_t2
...
; draw_to_front / draw_to_back: point both byte-aligned engines
; (hgr_blit2's bl_page + hgr_text8's ht_page) at the visible page
; (modals, prompts) or at the hidden one (turn renders).
; ----------------------------------------------------------------------------
draw_to_front:
        LDA     shown_page
        STA     bl_page
...
; ----------------------------------------------------------------------------
; page_flip: display the page that was just rendered (bl_page), then
; swap the per-page dirty snapshots so prev_vis / ent_prev describe the
; NEW back page's last frame. Soft switches are READ-only.
; ----------------------------------------------------------------------------
page_flip:
        LDA     bl_page
        BNE     @show2
...
; ----------------------------------------------------------------------------
; update_camera: CENTRE-LOCKED viewport — the player pins viewport cell
; (3, 2) and the world scrolls around them. The camera origin may sit
; past the map edges (8-bit wrap is fine: the render loop bounds-tests
; every map coordinate and blanks off-map cells). Every player move
; shifts the whole playfield -> force_dirty_all; free actions (bump,
; rest, throw) keep the camera still, so the delta renderer still
; skips clean cells there.
; ----------------------------------------------------------------------------
update_camera:
        LDA     player_col
        SEC
...
; ----------------------------------------------------------------------------
; vp_cell_pos: A = map col, Y = map row -> C clear + bl_col/bl_sl set if
; the cell sits inside the 8x5 viewport, C set otherwise. Preserves X.
; ----------------------------------------------------------------------------
vp_cell_pos:
        SEC
        SBC     cam_col
...
; ----------------------------------------------------------------------------
; dirty_test_a: A = linear cell index -> A != 0 iff the cell is in the
; dirty set (vis XOR prev) | ent_prev | ent_now. Preserves X.
; ----------------------------------------------------------------------------
dirty_test_a:
        PHA
        AND     #7
...
; ----------------------------------------------------------------------------
; blit_tile_x2_a / blit_sprite_x2_a: x2 colour pack blits (128 B/asset,
; 4 bytes x 32 rows via hgr_blit4). Tiles keyed by tile_char_base*32,
; sprites by their TMS slot*32. blit_sprite_x2_a keeps the caller's
; bl_mode (OR / hurt-flash); tiles STORE.
; ----------------------------------------------------------------------------
blit_tile_x2_a:
        TAY
        LDA     tile_char_base,Y
...
; ----------------------------------------------------------------------------
; text_color_a: A = HUE_* -> hgr_text8's artifact-colour attributes.
; Same colour model as the sprite packs (parity masks + palette bit);
; HUE_WHITE is the full-density pass-through.
; ----------------------------------------------------------------------------
HUE_WHITE  = 0
HUE_GREEN  = 1
HUE_ORANGE = 2
...
hue_cmask_ev:   .byte $7F, $2A, $2A, $55, $55
hue_cmask_od:   .byte $7F, $55, $55, $2A, $2A
hue_cbit:       .byte $00, $00, $80, $00, $80

; ----------------------------------------------------------------------------
; force_dirty_all: make the next render_tiles repaint every cell —
; modal exits (redraw_game) where arbitrary text covered the playfield.
force_dirty_all:
        ; Under the (vis XOR prev) delta formula, prev=$FF alone only
        ; dirties the DARK cells — the all-cells force channel is
        ; ent_prev, so fill both.
        LDA     #2
        STA     vp_force                ; off-map border: both pages
        LDA     #$FF
        LDX     #VIS_BUFFER_SIZE
@lp:    DEX
        STA     prev_vis,X
...
; ent_mark_a: OR the ent_now bit for linear cell index A (0..159) —
; the entity pre-scan of place_all_sprites marks every cell an entity
; occupies this frame (the thrown dagger included, which is the one
; entity that can sit on an UNLIT cell). Clobbers A, Y; preserves X.
ent_mark_a:
        PHA
        AND     #7
        TAY
...
; ----------------------------------------------------------------------------
; render_map (HGR): kept as an alias so every historical call site
; (`JSR render_map` immediately followed by `JSR place_all_sprites`)
; still assembles — but the real repaint now lives in
; place_all_sprites, which must ALWAYS redraw tiles under the moved
; entities (no hardware sprite plane to float them on). The alias is a
; no-op; place_all_sprites starts with JSR render_tiles.
; ----------------------------------------------------------------------------
render_map:
        RTS
...
; ----------------------------------------------------------------------------
; render_tiles (x2): repaint the DIRTY cells of the 8x5 viewport as
; 28x32 px coloured tiles. Per-cell bit tests (dirty_test_a /
; vis_test_a) replace the x1 build's row shift registers — 40 cells of
; probes cost ~2.5k cycles, noise next to one 128-byte blit. Dirty set,
; FOV gate, hidden-pit rule and the prev/ent snapshot are identical to
; the x1 build.
; ----------------------------------------------------------------------------
render_tiles:
        LDA     #0
        STA     rt_row                  ; viewport row 0..4
@row_lp:
        LDA     #0
        STA     rt_col                  ; viewport col 0..7
@col_lp:
        LDA     rt_row
        CLC
        ADC     cam_row
        ...
        ADC     cam_col
        STA     vp_mc
        ; centre-locked camera: the window can hang past the map — an
        ; off-map cell has no vis/map data, it just renders black (only
        ; when vp_force says the border shifted; otherwise it is black
        ; already and the probe is skipped).
        LDA     vp_mr
        CMP     #LOGICAL_ROWS
        BCS     @off_map
        LDA     vp_mc
...
@dirty:
        ...
        JSR     vis_test_a
        BNE     @lit
        LDA     #TILE_EMPTY             ; dirty + dark -> re-blank
        JMP     @draw
@lit:
        ; Fetch the packed nibble: byte = mr*8 + mc/2.
        LDA     vp_mc
        LSR
        STA     tmp_packed
        LDA     vp_mr
...
@have_nib:
        ...
        AND     #TILE_REVEAL_BIT
        BNE     @show
        LDA     #TILE_EMPTY             ; hidden pit -> blank floor
        JMP     @draw
@show:  TYA
@draw:
        STA     rt_tile
...
@row_done:
        ...
        JMP     @row_lp
@snapshot:
        LDA     vp_force                ; 2-render countdown: the border
        BEQ     @vp_ok                  ; must be blanked once per page
        DEC     vp_force
@vp_ok:
        ; prev_vis = vis_buffer, ent_prev = ent_now (this frame becomes
        ; "last frame" for both the FOV delta and the entity cells).
        LDX     #VIS_BUFFER_SIZE
@cp:    DEX
        LDA     vis_buffer,X
        STA     prev_vis,X
...
@lp:    DEX
        STA     vis_buffer,X
        BNE     @lp
        ; HGR port: a fresh dungeon means the whole playfield must be
        ; repainted regardless of what was lit before — prime the dirty
        ; snapshot to all-ones.
        JMP     force_dirty_all
...
; ----------------------------------------------------------------------------
; place_all_sprites (HGR): repaint the dirty playfield cells, then
; soft-blit every visible entity over its tile. Keeps the TMS name +
; call contract (every historical call site runs the full repaint).
; Slot numbers in MON_NAME / item_sprite_table are unchanged — they
; index rogue_sprites_hgr at slot*8 via blit_sprite_a.
; FOV gates are identical to the TMS version (vis_test_a on the linear
; cell index); the hurt flash renders as an inverted box (bl_mode 1).
; HUD icons (dagger ammo + buff timers) moved to draw_hud_icons, called
; from the tail of update_hud so the HUD text can't overwrite them.
; ----------------------------------------------------------------------------
place_all_sprites:
        JSR     draw_to_back            ; double buffer: render hidden
        JSR     update_camera           ; may force a full repaint
        ; --- Entity pre-scan: mark every cell an entity will occupy
        ; THIS frame in ent_now (FOV-gated like the draws, except the
        ; player and the thrown dagger, which render regardless). The
        ; cells entities occupied LAST frame sit in ent_prev — together
        ; with the FOV ring delta that is the whole dirty set.
        LDA     #0
        LDX     #VIS_BUFFER_SIZE
@ez:    DEX
        STA     ent_now,X
...
@em_live:
        ...
        STA     tmp
        JSR     vis_test_a              ; dark monsters aren't drawn —
        BNE     @em_mark                ; don't dirty their cells either
        JMP     @em_next
@em_mark:
        LDA     tmp
        JSR     ent_mark_a
        ...
        JMP     @em_next
@em_boss:
        LDA     tmp                     ; 2x2 footprint: E, S, SE cells
        CLC
        ADC     #1
        JSR     ent_mark_a
        LDA     tmp
...
@escan_done:
        ...
        ; --- Player (always lit — FOV is centred on them) ---
        LDA     #0
        STA     bl_mode
        LDA     player_hurt
        BEQ     @p_ok
        LDA     #3                      ; PALFLIP: palette-bit hurt flash
        STA     bl_mode
@p_ok:
        LDA     player_col
        LDY     player_row
        JSR     vp_cell_pos             ; camera keeps the player framed
        BCS     @p_off
        LDA     #SPRITE_NAME_PLAYER
        JSR     blit_sprite_x2_a
@p_off:

        ; --- Monsters (one blit per live, FOV-lit pool entry) ---
        LDX     #0
@mon_lp:
        LDA     monsters+MON_TYPE,X
        BNE     @mon_alive
...
@one_cell:
        ...
        CLC
        ADC     monsters+MON_COL,X
        JSR     vis_test_a              ; preserves X
        BNE     @mon_lit
        JMP     @mon_skip               ; dark cell -> not drawn
@mon_lit:
        LDA     #0
        STA     bl_mode
        LDA     monsters+MON_HURT,X
        BEQ     @m_col
        LDA     #3                      ; PALFLIP: green<->orange etc.
        STA     bl_mode
@m_col:
        LDA     monsters+MON_COL,X
        LDY     monsters+MON_ROW,X
        JSR     vp_cell_pos
        BCS     @mon_skip               ; lit but outside the viewport
        LDA     monsters+MON_NAME,X
        AND     #$FC                    ; defensive: slots are multiples of 4
        JSR     blit_sprite_x2_a        ; (blit engine never touches X)
        JMP     @mon_skip
@boss_paint:
        ; FOV gate on the anchor cell, like the TMS version.
        LDA     monsters+MON_ROW,X
        ASL
        ASL
        ASL
...
@boss_lit:
        ...
        LDA     monsters+MON_HURT,X
        BEQ     @b_col
        LDA     #3                      ; PALFLIP: the orange demon
        STA     bl_mode                 ; flashes blue
@b_col:
        ; x2: the 2x2-cell demon is 56x64 px = two 4-byte column halves.
        ; Drawn only when the whole footprint fits the viewport (anchor
        ; at vc <= 6, vr <= 3) — the dead-zone camera keeps an adjacent
        ; fight fully framed anyway.
        LDA     monsters+MON_COL,X
        SEC
        SBC     cam_col
        BMI     @b_off
...
@mon_done:
        ...
        ; --- Floor items (FOV-gated; OR-blits merge with anything on
        ;     the cell, which reads as "loot under feet") ---
        LDA     #0
        STA     bl_mode
        LDX     #0
@item_lp:
...
@item_done:
        ...
        ; --- Thrown dagger (mid-flight frames). It may cross UNLIT
        ;     cells, so mark its cell dirty for the next repaint. ---
        LDA     throw_active
        BEQ     @done
        LDA     #0
        STA     bl_mode
        ...
        LDY     cur_y
        JSR     vp_cell_pos
        BCS     @done                   ; flew out of the viewport
        LDA     #SPRITE_NAME_DAGGER
        JSR     blit_sprite_x2_a        ; its cell is in ent_now (pre-scan)
@done:
        RTS
...
; ----------------------------------------------------------------------------
; draw_text: paint an $FF-terminated string at a given name-table addr.
;   On entry:
;     A = VRAM addr low byte, X = addr high byte (with or without $40)
;     vdp_src_lo / vdp_src_hi = pointer to the source string
; HGR port: the address decodes to the text cursor (set_cursor_ax);
; hgr_putc8 preserves Y so the stream index survives each glyph.
; ----------------------------------------------------------------------------
draw_text:
        JSR     set_cursor_ax
        LDY     #0
...
; ----------------------------------------------------------------------------
update_hud:
        ; HGR port: a full HUD repaint is ~130 glyphs + 5 icon blits
        ; (~100k cycles). Skip it when no displayed value changed and no
        ; transient prompt / full wipe happened since the last paint.
        JSR     hud_dirty_check         ; C set -> repaint needed
        BCS     @paint
        RTS
@paint:
        JSR     clear_msg_rows          ; wipe rows 22-23 (stale prompt /
        LDA     #0                      ; stale timer digits) before the
        STA     hud_msg                 ; repaint lays them down again
        STA     hud_force
        LDA     #HUE_ORANGE             ; row 20: DEPTH in orange
        JSR     text_color_a
        LDA     #4                      ; text (20, 0) -> byte col 4
        STA     ht_col
        LDA     #160                    ; scanline 20 * 8
        STA     ht_sl
        ; --- "DEPTH " (6 chars) ---
        LDX     #0
...
; ----------------------------------------------------------------------------
update_hud_equip:
        LDA     #HUE_BLUE               ; row 21: combat stats in blue
        JSR     text_color_a
        LDA     #21
        STA     vdp_row
        LDA     #0
...
; ----------------------------------------------------------------------------
update_hud_hp:
        LDA     #HUE_GREEN              ; row 22: HP in green
        JSR     text_color_a
        JSR     tms9918_pad18   ; +18c silicon-strict pad18-v4 (before LDA #imm bridge)
        LDA     #22
        STA     vdp_row
...
; ----------------------------------------------------------------------------
update_hud_timers:
        LDA     #HUE_VIOLET             ; row 23: buff timers + XP in violet
        JSR     text_color_a
        LDA     #6                      ; text (23, 2) -> byte col 6
        STA     ht_col
        LDA     #184                    ; scanline 23 * 8
        STA     ht_sl
...
@xp_sp_lp:
        ...
        LDA     player_xp
        JSR     print_byte_3digits
        LDA     #HUE_WHITE              ; everything after the HUD (modals,
        JSR     text_color_a            ; prompts, death screen) is white
        ; Fall through into draw_hud_icons — the icon blits must land
        ; AFTER the HUD text pass, or the space-padding of rows 20-23
        ; would erase them (no hardware sprite plane to float them on).
        JMP     draw_hud_icons


; ----------------------------------------------------------------------------
; draw_hud_icons: soft-blit the HUD pictograms the TMS version carried
; as hardware sprites — the always-on dagger-ammo icon at text cols
; 28-29 rows 20-21 (scanline 160), and one 14x16 buff icon per ACTIVE
; timer at scanline 176 (rows 22-23), pixel cols 0/32/64/96 -> byte
; cols 4/8/12/16. Companion digit pairs are painted by
; update_hud_timers just before we run.
; ----------------------------------------------------------------------------
draw_hud_icons:
        LDA     #0
        STA     bl_mode               ; OR over the (blank) HUD cells
        LDA     #32                     ; text col 28 -> byte col 32
        STA     bl_col
        LDA     #160
        STA     bl_sl
        LDA     #SPRITE_NAME_DAGGER
...
; ----------------------------------------------------------------------------
; hud_dirty_check: C set iff update_hud must repaint — a prompt was
; printed (hud_msg), the screen was wiped (hud_force), or one of the 11
; displayed values differs from the last-painted copy in hud_cache.
; On "dirty" the cache is refreshed. hud_vars holds the ZP ADDRESS of
; each displayed value; LDA $00,X reads ZP[X].
; ----------------------------------------------------------------------------
hud_dirty_check:
        LDA     hud_force
        ORA     hud_msg
...
@cmp:   LDX     hud_vars,Y
        ...
        DEY
        BPL     @cmp
        ; values unchanged — but with double buffering a paint must
        ; land on BOTH pages: honour the pending second-page repaint.
        LDA     hud_again
        BEQ     @clean
        LDA     #0
        STA     hud_again
...
@cp:    LDX     hud_vars,Y
        ...
        BPL     @cp
        LDA     #1
        STA     hud_again               ; the other page needs it too
        SEC
        RTS

hud_vars:
        .byte   depth, hp, hp_max, player_dmg, player_def, player_xp
        .byte   dagger_qty, weapon_timer, armor_timer, ring_timer
        .byte   torch_timer
...
@no_torch_tick:
        ...
        STA     ring_boost
@no_ring_tick:
        ; update_hud owns the rows-22/23 wipe since the HGR repaint-
        ; elision pass: it clears them iff it repaints (a transient
        ; prompt sets hud_msg, which forces exactly one repaint+clear).
        JSR     tms9918_pad18   ; +18c silicon-strict pad18-v4 (back-to-back VDP store)
        JSR     update_hud
        JSR     page_flip               ; present the finished frame
        LDA     hp
        BNE     @alive
...
death_screen:
        ...
        JSR     draw_to_front           ; splash: draw on the shown page
        JSR     clear_name_table
...
@d_inner:
        ...
        JMP     start                   ; cold-start (was JMP $4000 on the cartridge)
...
; ----------------------------------------------------------------------------
redraw_game:
        ; HGR port: no full framebuffer clear (that would flash black on
        ; every modal exit AND every dagger-flight frame). render_tiles
        ; STORE-repaints the playfield, update_hud repaints rows 20-21
        ; fully; only rows 22-23 need the explicit text wipe. The modal
        ; text that covered the playfield forces a full dirty set.
        JSR     force_dirty_all
        JSR     clear_msg_rows
        JSR     render_map
        JSR     place_all_sprites
        JSR     tms9918_pad18   ; +18c silicon-strict pad18-v4 (back-to-back VDP store)
        JSR     update_hud
        JMP     page_flip               ; present the rebuilt frame
...
; ----------------------------------------------------------------------------
show_inventory:
        ; HGR port: the TMS version pre-built a SAT of item pictograms
        ; here; the icons are now blitted per line by draw_inv_icon
        ; (called from the slot loop below, AFTER each line's text).
        JSR     draw_to_front           ; modals draw on the shown page
        JSR     clear_name_table
...
@slot_lp:
        ...
        JSR     draw_inv_line
        LDX     tgt_col
        JSR     draw_inv_icon           ; blit the pictogram at col 10
...
; ----------------------------------------------------------------------------
; draw_inv_icon: blit one bag slot's 14x16 item pictogram at text col
; 10 (byte col 14), vertically straddling the entry's two display rows
; at scanline (row - 1) * 8 — same placement as the TMS hardware
; sprite. X = slot byte offset (preserved: the blit engine never
; touches X); map_ptr = the entry's display row.
; ----------------------------------------------------------------------------
draw_inv_icon:
        LDA     map_ptr
        SEC
        ...
        ASL
        ASL
        ASL                             ; (row - 1) * 8 = top scanline
        STA     bl_sl
        LDA     #14                     ; text col 10 -> byte col 14
        STA     bl_col
        LDA     #0
        STA     bl_mode
        LDA     inventory+INV_TYPE,X
...
print_msg_row:
        PHA
        JSR     draw_to_front   ; prompts land on the page being shown
        LDA     #1
        STA     hud_msg         ; rows 22-23 now carry a transient prompt
        PLA
        STA     vdp_row
        LDA     #0
        STA     vdp_col
...
@map_lp:
        ...
        STA     vis_buffer,X
        BNE     @map_lp
        JSR     clear_msg_rows          ; HGR port: reveal repaints every
        JSR     render_map              ; cell anyway (vis all-ones)
        JSR     place_all_sprites
        JSR     tms9918_pad18   ; +18c silicon-strict pad18-v4 (back-to-back VDP store)
        JSR     update_hud
        JSR     page_flip               ; present the revealed map
...
; ----------------------------------------------------------------------------
delay_throw_frame:
        LDX     #48                     ; HGR port: the flight frame is a
                                        ; dirty repaint (~2 cells) since
                                        ; the entity-delta pass — 48 laps
                                        ; ≈ 60 ms restores the TMS pacing
@d1:    LDY     #0
@d2:    INY
        BNE     @d2
        DEX
...
@have_dagger:
        ...
        RTS
@have_dir:
        ; Wipe the "DIRECTION? " prompt once — the flight frames repaint
        ; only dirty cells now, so the prompt would otherwise linger for
        ; the whole flight (the TMS build wiped it via its per-frame
        ; full-screen redraw).
        JSR     clear_msg_row23
...
@flight_lp:
        ...
        BCC     @hit_mon
        ; Empty cell — render this frame and step again. Dirty repaint
        ; only (the dagger's previous cell sits in ent_prev, its new one
        ; enters ent_now): ~2 cells + entities instead of the forced
        ; full-screen redraw_game the modal exits use.
        JSR     place_all_sprites
        JSR     page_flip               ; present the flight frame
...
@vanish:
        ; Dagger ends here — clear the in-flight flag, repaint once so
        ; the projectile disappears (its last cell is in ent_prev),
        ; charge a turn. finish_turn repaints the HUD if anything moved.
        LDA     #0
        STA     throw_active
        JSR     place_all_sprites
        JSR     page_flip               ; present the impact frame
        LDA     #1
        RTS
...
show_help:
        ...
        WAIT_VBLANK_SAFE
        JSR     disable_sprites
        JSR     draw_to_front           ; modal: draw on the shown page
        JSR     clear_name_table
...
; ----------------------------------------------------------------------------
draw_title:
        ; x2 banner: "ROGUE" doubled to 16x16 glyphs, orange, centred on
        ; text rows 2-3 (the x1 "ROGUE" line left the table).
        LDA     #HUE_ORANGE
        JSR     text_color_a
        LDA     #15                     ; (40 - 5*2) / 2
        STA     tx2_col
        LDA     #16                     ; scanline: text row 2
        STA     tx2_sl
        LDX     #0
@x2lp:  LDA     title_rogue,X
        CMP     #$FF
        BEQ     @x2done
        STX     tmp                     ; A carries the char — save the
        JSR     putc8_x2                ; index in ZP, not via TXA/PHA
        LDX     tmp
        LDA     tx2_col
        CLC
        ADC     #2
        ...
        BNE     @x2lp
@x2done:
        LDA     #HUE_GREEN              ; title body in green
        JSR     text_color_a
...
@lp:    LDA     title_table+0,X         ; str_lo
        ...
        JSR     tms9918_pad18   ; +18c silicon-strict pad18-v4 (back-to-back VDP store)
        BCC     @lp
        LDA     #HUE_WHITE              ; briefing / dungeon text is white
        JMP     text_color_a
...
; ----------------------------------------------------------------------------
; putc8_x2: draw char A doubled to 16x16 px at byte column tx2_col,
; top scanline tx2_sl, through the text colour attributes (parity mask
; by ABSOLUTE byte-column parity + palette bit — doubled pixels are
; 2-px runs, so one parity survives per pair: pure colour, shape
; intact). Font: the HGR-order bbfont already linked for the x1 text
; (HGR_Font5F, bit 0 = leftmost, 7 px/glyph). Doubling in HGR order is
; direct: dblnib is a pure bit-doubler (bit k -> bits 2k, 2k+1), so
;   left  byte = dblnib[row & $0F]  & $7F   (doubled px 0..6)
;   right byte = dblnib[row >> 3]  >> 1     (doubled px 7..13 — source
;                bit 3's pair straddles the byte boundary)
; Clobbers A, X, Y, bl_src, et_t/et_t2. Title-screen use.
; ----------------------------------------------------------------------------
putc8_x2:
        SEC
        SBC     #$20
        ...
        STA     tx2_row
@row:   LDY     tx2_row
        LDA     (bl_src),Y              ; HGR-order glyph row
        PHA
        AND     #$0F
        TAY
        LDA     dblnib,Y                ; doubled px 0..7 of the pair run
        AND     #$7F
        STA     tx2_l
        PLA
        LSR
        ...
        ASL
        CLC
        ADC     tx2_sl                  ; first scanline of the pair
        JSR     @putrow
        LDA     tx2_row
        ASL
        CLC
        ADC     tx2_sl
        CLC
        ADC     #1                      ; second scanline (vertical x2)
        JSR     @putrow
        INC     tx2_row
        LDA     tx2_row
        CMP     #8
...
; nibble -> byte with every bit doubled (bit k -> bits 2k, 2k+1 — works
; for either bit order; used here on HGR-order rows)
...
; ============================================================================
; HGR asset pack (generated) + GEN2 lib modules. Pulled in last so the
; data tables sit at the tail of the CODE segment.
;   rogue_assets_hgr.inc : 8 tiles + 14 sprites (14x16) + 28x32 boss,
;                          converted from the TMS art by
;                          tools/build_rogue_hgr_assets.py
;   hgr_scanline.inc     : hgr_lo/hgr_hi scanline base tables
;   bbfont_ascii5f.inc   : HGR_Font5F 8x8 glyphs ($20-$5F)
;   hgr_clear.asm        : clear_hgr (uses our ptr_lo/ptr_hi ZP)
;   gen2_init.asm        : gen2_hgr_init_clear (blank-first HGR init)
; ============================================================================
.include "rogue_assets_hgr.inc"
.include "rogue_assets_hgr_x2.inc"
