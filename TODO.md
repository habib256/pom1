...
>
> **Note de conception : [`doc/TELEMETRY_SIDE_CHANNEL.md`](doc/TELEMETRY_SIDE_CHANNEL.md).** **✅ Fonctionnellement complet → [`CHANGELOG.md`](CHANGELOG.md)** : `TelemetryPort` (`$C440-$C443`, FIFO in/out, serveur TCP, `KeyInjector`), bus priorité 30 (zone A9=0 aveugle à GEN2), `--telemetry-port N`, **lock-step déterministe** (parke le CPU via `M6502::stop()` jusqu'à l'ACK `0x06`, vérifié end-to-end + `tools/test_telemetry_lockstep.py`), `--telemetry-log PATH` (golden-trace), panneau UI (*View → Telemetry Side Channel…*), **`--headless`** (exécution sans GLFW/display — `tools/test_telemetry_lockstep.py` passe en CI sans écran). La boucle « régression automatique » du SDK de Bernie est **complète côté émulateur** ; l'umbrella restant est **CI GitHub Actions** (section Technical debt, ci-dessous). **Kit SDK livré** : lib cc65 `dev/lib/telemetry/telemetry.inc` + lib harnais Python `tools/pom1_telemetry.py` + démo `dev/projects/a1_telemetry_demo/` & worked-example `tools/test_telemetry_demo.py` (boucle compile→load→test vérifiée headless). Restes ci-dessous (le reste du « dream SDK » = **CI GitHub Actions** + **CPU cycle-exact**, déjà suivis en Technical debt).

- [ ] **Presets / cartes en `--headless`** `[M · nice]` — aujourd'hui `--headless` ne monte que la machine 64K (`applyMachineConfig` couplé ImGui). Extraire la config machine (RAM + enables cartes via façades `EmulationController`) pour tester des **jeux HGR GEN2** sans écran — le vrai trou pour les jeux graphiques.
