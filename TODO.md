...
- [ ] **WASM cc65 — arbitrer le port emscripten** `[L · nice]` — aujourd'hui le build web garde le **Bench Woz-hex** + un CTA « desktop only — download the app » (`IBenchHost::headerNote()`) ; le chemin asm/C est absent (pas de subprocess en WASM). 8bitworkshop **prouve la faisabilité** (cc65/ca65/ld65 compilés en WASM via Emscripten) et c'est le levier qui ferait tourner le Bench dans le **canal web zéro-friction** (~80 % Windows + WASM = là où le toolchain natif passe le plus mal). Soit un port emscripten (gros chantier), soit on assume le CTA comme réponse définitive — à reconsidérer, pas à lancer tête baissée.
