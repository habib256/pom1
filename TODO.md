...
- [ ] **GEN2 : montrer chaque trame émulée, une par rafraîchissement d'écran** `[M · solid]` — Uncle Bernie (16 sept. 2026) : le défilement fin de sa démo `vsplits` *« is staggering along, as if it could not do 60 frames per second »*. Ce n'est pas l'émulation : `gen2_vsplits_smoke` prouve une ligne par trame émulée, sans exception, et `GraphicsCard::render()` coûte ~24 µs par trame. C'est la présentation. Le thread d'émulation publie après chaque tranche (~1 ms), l'UI prend **la dernière** trame complète à chaque vsync ; la trame GEN2 émulée dure 17 030 cycles (60,11 Hz au rythme actuel du « x1 », 60,05 Hz au vrai), l'écran 60 Hz, et la moindre gigue près de la frontière fait voir 0 ou 2 trames d'un coup. Simulé : **4 % des images** sautent ou répètent une ligne avec 0,5 ms de gigue UI, **27 %** avec 4 ms — ce qu'un portable de 2012 fournit facilement. Correctif : un petit anneau de trames GEN2 publiées (verrou de trame + journal + pages texte, numérotées) et une UI qui avance d'**une** trame par vsync, en ne sautant ou répétant qu'à la dérive des horloges (une fois toutes les ~9-18 s au lieu de rafales). Contrainte : la façade `Memory` est gelée — l'anneau vit dans `Gen2VideoScanner`, comme le journal. Le verrou de trame ne couvre aujourd'hui que `$2000-$5FFF` : les pages TEXT/LORES (`$0400-$0BFF`) sont lues en direct au rendu, donc déchirables — le même anneau le corrige.
- [ ] **« x1 » cadence à 1,02372 MHz, pas à 1,022727** `[S · solid]` — `runEmulationSlice` fait `cyclesPerSecond = cpf × 60.0`, avec `cpf = POM1_CPU_CYCLES_PER_FRAME_1X_60HZ` = 17 062, qui est le nombre de cycles d'une trame **à 59,94 Hz** (`CpuClock.h`). Résultat : +0,097 %, alors que l'infobulle promet 1,022727 MHz. Multiplier par `60000.0 / 1001` au lieu de `60.0` ; la trame TMS9918 (17 062 cycles) tombe alors vraiment à 59,94 Hz. Vérifier `measured_cpu_rate_smoke` et les harnesses telnet, qui mesurent en temps mural.
...
- [ ] 🚫 **Le vrai jeu de caractères de la GEN2** `[S · solid]` — attendre le gabarit texte qu'Uncle Bernie propose dans son PDF (*« I can send you the character set template I use »*). Son EPROM laisse la ligne blanche de chaque cellule **en haut** (comme le 2513 de l'Apple II), les glyphes IIe de POM1 la laissent en bas, avec les jambages des minuscules en ligne 7 — décaler d'une ligne casserait `g j p q y`, donc pas de décalage deviné. À réception : un convertisseur gabarit → `roms/gen2_char.rom`, chargé avant la ROM IIe. `gen2_chargen_smoke` couvre déjà les équivalences du tableau 2 pour n'importe quel dessin ; ajouter la position de la ligne blanche.
