...
> **Interpréteur V2.6 + injection + 10 sketches + REPL interactif livrés** → `[CHANGELOG.md](CHANGELOG.md)`. LOGO est le **4ᵉ langage** du dialogue *New* (`kP1Languages[]` « LOGO — injected listing »), deux cibles interpréteur (LOGO TMS9918 idx 14, cold `4000R`, `Codetank_GAME3.rom` bank Lower ; LOGO GEN2 HGR idx 15, cold `6000R`, `roms/logo-gen2.rom`). Chemin `injectLogo()` (mode 6) + `LogoProgramLoader` C++ pur (peuple `proc_table` en source ASCII brut + `n_procs`, feed d'**une** ligne d'entrée) ; coloration `langLogo()` ; pin `bench_logo_inject_smoke` (`proc_table` `$E431`/`$B431`, `n_procs` `$0260`/`$02E3`). Après Run, une **entrée REPL** (`IBenchHost::replActive/replSend`) feed les lignes tapées live une à une vers le REPL résident (ligne envoyée échoée dans la console ; la sortie texte de l'interpréteur — prompt, `OK`, `PRINT`, erreurs — reste sur l'**écran Apple-1**, à sa place). WASM-safe.
>
> Reste ouvert (nice-to-have) :
> - [ ] **Break (ESC/Ctrl-G) depuis le REPL** `[S · nice]` — le champ REPL ne poste que des lignes imprimables + CR ; ajouter un bouton/raccourci qui `queueKey` `$1B`/`$07` pour interrompre un `REPEAT FOREVER` sans passer par le Stop CPU global (qui fige tout).
> - [ ] **Read-back / LIST round-trip** `[M · nice]` — l'inverse de `LogoProgramLoader` (proc_table résident → source), pour « récupérer depuis la machine » ce qui a été défini au REPL ; prérequis d'un inject à chaud incrémental (append de procs sans cold-reset).
