...
- [x] **CodeTank daughterboard ROM — fixed 16 kB window** `[S · nice]` — BOM v0.4 (28c256 + 74LS156 + 3-pin jumper + 2×22 header) is not paged like Juke-Box: it maps one contiguous 16 kB half at `$4000-$7FFF`, with the board jumper selecting lower/upper half of `roms/codetank.rom`.
- [x] **CodeTank as standalone card paired with TMS9918** `[M · solid]` — Split CodeTank out of the JukeBox class into its own `CodeTank.h/.cpp` peripheral, its own Hardware menu entry / `CT` toolbar button / Hardware window, and a dedicated **CodeTank Library** window that scans `roms/codetank/*.{rom,bin}` (32 kB images with optional `.txt` sidecar) and lets the user pick which 16 kB half to wire in. Mutex only with the Juke-Box (overlapping ROM window) — coexists freely with TMS9918 so a CodeTank ROM can ship a TMS9918 game that runs unchanged on real silicon. CLI: `--enable codetank`, `--codetank-jumper lower|upper`, `--codetank-rom <path>`. Pinned by `tests/codetank_smoke_test.cpp`.
