...
- [ ] **Movies (issue #40) : re-record et export longplay** `[M · solid]` — l'enregistrement et le rejeu vérifié existent (`InputMovie.h`, `input_movie_smoke`). Reste : (1) re-record — pendant un enregistrement, un retour arrière (rewind) ou un chargement de snapshot coupe le journal à ce cycle et l'enregistrement reprend de là ; (2) `--movie-play` en headless, qui rejoue sur un contrôleur `Deterministic` ; (3) l'export longplay — une séquence d'images (écran Apple-1, GEN2, TMS9918) et le son, vers ffmpeg. Les resets pendant l'enregistrement pourraient devenir des événements du movie plutôt qu'une divergence.
