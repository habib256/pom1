...
- [x] **TMS9918 Sokoban** — Bundled as `software/tms9918/TMS_Sokoban.asm` and `TMS_Sokoban.txt` (4354 bytes, `$0280-$1381`). P-LAB Apple-1 Graphic Card version: Graphics I mode, 32×24 display with 8×8 character tiles. 7 tile types, one per "colour group" (chars 0, 8, 16, 24, 32, 40, 48) so each type gets its own palette colour (grey wall, red target, yellow box, light-green box-on-target, light-blue player, medium-green player-on-target, transparent floor). 20×12 playfield centred at (row 6, col 6). Delta rendering: each move sets a new VRAM address on `$CC01` and writes 1-4 char codes to `$CC00`. Same state grid / game logic as text and HGR ports. 47 levels (3 teaching + Microban I #1..#44). Apple 1 text screen carries title, layout prompt (QWERTY/AZERTY) and win messages. Requires the TMS9918 card to be enabled in the Hardware menu. Load via `280R`.
