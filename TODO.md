...
**Bouton du Bench câblé (2026-06-17, 3ᵉ passe) — chemin asm.** Plutôt qu'ASYNCIFY (coût sur tout le binaire + risque sur la main-loop GLFW), l'async passe par une **machine à états / poll** (rayon de casse minimal, desktop reste 100 % synchrone) : `BuildResult` gagne `pending`, `IBenchHost::pollBuild()` (no-op par défaut), `CodeBench` poll chaque frame tant que `pending`. `Pom1BenchHost` (WASM) : `available()` expose les cibles **asm** (dual-4k / GEN2 HGR / GEN2 TXT) + hex/raw ; `build()` lance `window.POM1cc65.buildAsm` via `EM_ASM` (écrit `.bin`/`.log` + `Module.__benchJob` dans MEMFS) et renvoie `pending` ; `pollBuild()` relit le résultat, route le log via `parseErrorMarkers`/`humanizeCc65`, puis `loadBinary`+run à l'entrée du cfg. `toolchainReady/Hint/headerNote` MAJ. **Desktop + WASM compilent proprement** (desktop inchangé : `pending` jamais positionné → poll inerte). → **le bouton Verify/Run du Bench web assemble + exécute du 6502 en navigateur.**
**Chemins TMS9918-asm + C câblés (2026-06-17, 4ᵉ passe).** TMS9918 asm : `pollBuild()` flashe le `.bin` en ROM CodeTank (4000R). Chemin **C** : `build_cc65_wasm.sh` native-build `none.lib` depuis la même source cc65 (le WASM cc65 émet l'ABI zp `c_sp` → un `none.lib` ancien `sp` ne linke pas) + stage `asminc/` (fichiers `.macpack` que ca65 ouvre — le `.s` généré fait `.macpack longbranch`), `include/`, `lib/none.lib` dans `build-wasm/cc65/` ; CMake les préload sous `/cc65`. `cc65_bench.js` `buildC` : cc65 par `.c` → ca65 par `.s` (seed asminc) → ld65 + `none.lib` ; `build()` fournit le spec par cible (la cible C-gen2 embarque aussi la couche gfx). `available()` expose **toute la matrice** (4 asm + 4 C + hex/raw) ; le CTA `headerNote` « desktop only » retiré. **Vérifié byte-identique au cc65 natif 2.19** : plain-C (399 o) + GEN2-C multi-fichiers+gfx (9557 o). Desktop + WASM compilent ; `POM1.data` 52 Mo (runtime C inclus).
**Bouton « New » du Bench corrigé en WASM (2026-06-17, 5ᵉ passe).** Le constructeur WASM exposait les cibles mais laissait la matrice New (langage × machine) vide → `doNewChoose()` retombait sur `doNew()` (recharge le starter courant, pas de sélecteur). Désormais `languages_/machines_/hints_` peuplés aussi en WASM (les starters sont des chaînes compilées, pas d'accès fichier ; cibles exposées en map-identité → `targetFor`→`applyTarget` résout ; `onTargetSelected` change déjà le preset en WASM). Les *Examples* restent desktop-only (leurs sources `dev/projects/` ne sont pas préloadées).
