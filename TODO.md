...
**Reste (documenté `doc/CC65_WASM.md`)** : chemin **C** (cibles mode 3 + libs runtime cc65 2.19 — `build()` renvoie « desktop-only ») ; cible **TMS9918 asm** (flash ROM) ; **vérif navigateur** (ouvrir `POM1.html`, tester un sketch asm — non testable headless cette session, mais glue Node-vérifiée byte-identique + les deux builds compilent).
