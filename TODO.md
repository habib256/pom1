...
- [ ] **Disable keyboard autorepeat (TTL-accurate)** — real Apple-1 TTL keyboards fire STROBE once per keypress; holding a key does not retrigger. POM1 currently repeats via ImGui's `InputQueueCharacters` fed by `glfwSetCharCallback` (GLFW char callbacks fire on repeat). Shortcuts already gate on `GLFW_REPEAT` (`MainWindow_Keyboard.cpp:83`, F7 exempted for step-CPU), but typed characters slip through. Add `bool keyboardAutoRepeat = true;` (default on — modern UX) in `MainWindow_ImGui.h:70-82`, hook `glfwSetCharCallback` before ImGui (or track per-key press state via `glfwGetKey`) to drop repeats when the toggle is off, and expose via the Hardware menu (`MainWindow_Menu.cpp` ~L.186). Simpler fallback: per-key timing threshold (~50 ms) in `handleKeyboardInput` (`MainWindow_Keyboard.cpp:45-67`) if the callback hook proves invasive. Source: feedback from Claudio Parmigiani — "given the lack of backspace function, autorepeat adds anxiety".
