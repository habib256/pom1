...
constexpr quint8 kAciRom[0x100] = {
    ...
    // Char comparisons need bit 7 set: $D010 reads latch the keyboard
    // strobe into bit 7 and the chars stored at $0200+ retain it (same
    // convention Wozmon uses at $FF58: CMP #$D2). Publicly circulating
    // ACI.rom dumps ship with bit 7 stripped here ($52/$57/$2E/$20), which
    // makes the parser reject every input char and restart at $C100 — see
    ...
    // EOR #$B0 (was #$30) — chars stored at $0200+ retain $D010's bit 7,
    // so the ASCII→nibble conversion has to absorb that bit instead of just
    // the high nibble. Wozmon's hex parser at $FF62 already does EOR #$B0
    // for the same reason; the canonical ACI dump shipping with EOR #$30
    // could only ever have worked against bit-7-stripped storage.
