...
module ascii_input(
   ...
   // MiSTer "Load Ascii" is intended for small text payloads; cap to 4 KiB.
   reg [11:0] ascii_last_byte = 12'b0;
   reg [11:0] text_byte = 12'b0;
   reg in_dl = 1'b0;

   wire [7:0] ascii_data_q;
   wire ascii_wr = ioctl_download && (textinput_addr[15:12] == 4'b0000);

   spram #(.addr_width(12), .data_width(8), .mem_name("ASCII")) ascii_ram
   (
      .clock(clk25),
      .address(ascii_wr ? textinput_addr[11:0] : text_byte),
      .data(textinput_dout),
      .wren(ascii_wr),
      .q(ascii_data_q),
      .cs(1'b1)
    ...
    begin
        ...
        begin
            ...
            begin
              ...
              if (!ascii_rdy & data_ready & (text_byte<=ascii_last_byte))
		        begin
				     case(ascii_data_q)
					  8'h0A: ascii=8'h0D;
					  default: ascii= ascii_data_q;
					  endcase
                 
            	  ascii_rdy   <= 1;
                 text_byte = text_byte + 12'd1;
		        end
            end
            ...
            //prev_ps2_clkdb <= key_clk;//ps2_clkdb;            
  	         ...
  	         if (ascii_wr)
            begin
                 ascii_last_byte = textinput_addr[11:0];
                 text_byte = 12'b0;
                 in_dl=1'b1;
            end
            else
