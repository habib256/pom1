...
**Uncle Bernie's GEN2 HGR** — **280×192** HIRES, Apple II-compatible memory at `$2000-$3FFF`, **NTSC artifact colour**. Its 18 programs live in `software/Graphic HGR/` — Uncle Bernie's own `vsplits.apl` among them as Woz-hex dumps — *File → Load Memory* (the folder auto-plugs the card), e.g. [HGR Maze](sketchs/gen2/game_maze/HGR_Maze.asm). Details → [`doc/GEN2_RELEASE.md`](doc/GEN2_RELEASE.md).
