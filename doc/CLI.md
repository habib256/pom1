...
| `--headless` | A | Run with **no GLFW window / GL / ImGui** — for CI & scripted runs on a display-less box. Default 64K machine + execution speed + telemetry + all Phase-C deferred verbs (`--load`/`--run`/`--paste`/`--step`/`--sd-*`/`--snapshot`/`--break`); idles until SIGINT/SIGTERM. **`--preset` / `--enable` / `--disable` are GUI-only and skipped** (logged). Pairs with `--telemetry-port` / `--telemetry-log` for headless regression tests (see `tools/test_telemetry_lockstep.py`). |
