...
| `--headless` | A | Run with **no GLFW window / GL / ImGui** — for CI & scripted runs on a display-less box. Default 64K machine + execution speed + telemetry + all Phase-C deferred verbs (`--load`/`--run`/`--paste`/`--step`/`--sd-*`/`--snapshot`/`--break`); idles until SIGINT/SIGTERM. Applies `--preset` / `--enable` / `--disable` (RAM + cards + BASIC ROM, plugged immediately) — e.g. `--preset 13` plugs GEN2 for headless HGR game tests. Pairs with `--telemetry-port` / `--telemetry-log` (see `tools/test_telemetry_lockstep.py`, `tools/test_headless_preset.py`). |
