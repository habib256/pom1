...
- **RAM expansion (Q9):** the card carries 48 KB DRAM covering `$0000-$BFFF`
  ...
  preset 8 models this configuration (48 KB + the `$E000` bank).
...
- **Preset 8** = Bernie's real machine: GEN2 (beam-raced, all modes,
  HST0-exact) + ACI + 48 KB. The `software/Graphic HGR/` folder auto-plugs
  the card on load.
- **Dev loop:**
  ```bash
  cd dev/projects/<yours> && make        # ca65 + ld65 + Woz-hex .txt
  ./build/POM1 --preset 8 --load 'E000:software/Graphic HGR/<P>.txt'
  ```
