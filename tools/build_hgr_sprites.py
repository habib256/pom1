...
def emit_hgr_asm(category: str, title: str,
    ...
    n = len(sprites)
    out.append("; " + "=" * 76)
    out.append(f"; sprites_{category}_hgr.asm  --  {n} sprites, GEN2 HGR x1 mono master")
    out.append("; " + "-" * 76)
    ...
    out.append(";")
    out.append("; REGIME: x1 mono. On HGR the colour is a pure NTSC ARTIFACT (Woz's")
    out.append("; trick), NOT a chosen colour. This is the compact 48-byte master; for")
    out.append("; a CHOSEN colour, double it to x2 in the POM1 HGR Sprite editor (per-")
    out.append("; project, 4x the bytes) or at runtime via gen2_hgr_inflate_x2.")
    out.append(";")
    out.append("; Layout: each sprite = 16 rows x 3 bytes = 48 B.")
    out.append("; HGR encoding: bit 0 = leftmost pixel within byte. Bit 7 = palette")
    out.append("; GROUP (0 = violet/green, 1 = blue/orange), emitted as group 0 here;")
    out.append("; the editor can flip it per sprite. 16-px sprite: cols 0..6 in byte 0,")
    out.append("; cols 7..13 in byte 1, cols 14..15 in bits 0..1 of byte 2.")
    out.append(";")
