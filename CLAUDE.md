...
- **MainWindow_ImGui** — single class across **9 TUs** sharing helpers via `MainWindow_Internal.h` (`pom1::mainwindow::detail`): `MainWindow_ImGui` (ctor/dtor, render, CPU control), `_Layout`, `_Presets` (`kMachinePresets[]`; migration target for external `presets.json`), `_Menu`, `_Dialogs` (About/Thanks/Welcome/Hardware/Display — texture loaders `ensureAboutPhotoTexture`/`ensureApple50LogoTexture`/`ensureAppIconTexture`, released in `destroyPom1()`), `_HardwareWindows` (GraphicsCard / TMS9918 / WiFiModem / TerminalCard / A1IO_RTC / Juke-Box; TMS9918 GL texture 256×192 RGBA `GL_NEAREST`), `_FileDialogs` (auto-enable-by-directory), `_DebugWindows`, `_Keyboard`.
  ...
  **`applyMachineConfig(int)` invariants**:
  ...
  - `applyPendingLayout(const char*)` runs before each `Begin()` with **`ImGuiCond_FirstUseEver`** — preset ships defaults, user drags persist to `ini/imgui_preset_NN.ini` (per preset, see below) and win on subsequent loads of that preset. **Widgets must not call `SetNextWindowSize(..., FirstUseEver)` themselves** — that runs after `applyPendingLayout` and overrides preset. `SetNextWindowSizeConstraints` is OK.
  - **Per-preset ini files** (`ini/imgui_preset_NN.ini` + `ini/preset_NN.size`). `io.IniFilename = nullptr` at boot disables ImGui's default global `imgui.ini`; POM1 manages per-preset ini files manually via `MainWindow_ImGui::{savePresetLayout,loadPresetLayout}`. The ini holds window positions/sizes (parsed by ImGui), the `.size` sidecar holds the GLFW OS-window `W H` dimensions so switching presets also restores the outer frame. `applyMachineConfig(N)` saves the outgoing preset's state and loads incoming preset's state before setting `activePresetIndex = N`. On clean shutdown `main_imgui.cpp` calls `savePresetLayout(activePresetIndex)` before `ImGui::DestroyContext()`. The `ini/` folder is created up front at boot. First-time use of a preset (no saved ini): `pendingLayout` + `FirstUseEver` + `computePresetLayoutExtent → glfwSetWindowSize` provide the default arrangement.
  - Default POM1 preset = last entry of `kMachinePresets[]` (*POM1 Multiplexing Fantasy (2026)*; picked via `kMachinePresetCount - 1`). Shipped layout: Screen (10,61) 843×701 / Welcome (858,61) 338×223 / Cassette Deck (858,288) 338×476. First-time use writes `ini/imgui_preset_14.ini` + `ini/preset_14.size`; the preset table is authoritative (see `MainWindow_Presets.cpp`'s Preset 14 description). Preset flips `showCassetteDeck = true` + `showWelcome = true`. **When adding a new preset, keep POM1 Fantasy last** — the default-picker uses the terminal index, and it also drives the banner (`screen->setShowBanner(presetIndex == kMachinePresetCount - 1)`).
  - First-time use of a preset (no saved `.size` sidecar): `glfwSetWindowSize` to `max(computePresetLayoutExtent(cfg, fallback), POM1 Fantasy extent)`, where the Fantasy floor (1206×807 = Screen 843×701 + right column 338 + chrome) is read from `kMachinePresets[kMachinePresetCount - 1]` — switching presets never shrinks the OS window; presets whose own layout demands more keep their extent-driven size. `glfwCreateWindow` hints 1274×801 to avoid a resize flash on boot; WASM mirrors this in `getWasmCanvasPixelSize`. Same "keep Fantasy last" invariant as the default-picker and banner.
...
- `$D012` (DSP) — write triggers display callback + busy-counter bump for every value except **raw $7F** (the WOZ Monitor reset-time `LDY #$7F / STY $D012` DDR-setup, which would otherwise paint a spurious `_` before the `\` prompt). Real hardware only latches on PB7=1, but POM1 is historically permissive so emulator-era demos (e.g. `software/tms9918/demo.bin`) that call WOZ ECHO with bit 7 clear still render correctly — keep this invariant when touching the DSP path. `$DF` via ECHO still yields `_` (`value & 0x7F = $5F`, the underscore slot in the Apple-1 character ROM), so the filter only drops the specific DDR write, not legitimate underscores. TerminalCard + PR-40 sniffers receive every `$D012` write unfiltered — they apply their own bit-7 / printable-range logic downstream. Read returns bit 7 = 0 (ready) after terminal-speed delay, else bit 7 = 1 (busy); counter clamped ≥ 0. PR-40 busy OR-merged on read when switch mode ≠ Off.
