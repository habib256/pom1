...
Two `ctest` targets, registered from `tests/CMakeLists.txt` (native-only, opt-out via `-DPOM1_ENABLE_TESTS=OFF`). Run from the build dir:

```bash
ctest                                    # both tests (~2 s total)
ctest --output-on-failure                # show stdout/stderr on regression
ctest -R klaus -V                        # Klaus only, verbose
```

**`klaus_6502_functional`** — runs [Klaus Dormann's 6502 functional test](https://github.com/Klaus2m5/6502_65C02_functional_tests) against the M6502 core. `file(DOWNLOAD)` fetches the 64 KB pre-assembled image once at configure time (SHA-256 pinned). The runner (`tests/klaus_6502_functional_test.cpp`) instantiates `Memory` with `setTestMode(true)` (flat RAM), copies the image in, sets `PC = $0400` via `M6502::setProgramCounter()` (the test's reset vector points at an error trap — callers must jump directly), and steps instruction-by-instruction until a `JMP *` trap is detected. Success = final PC equals `$3469`. Takes ~1.5 s at 200 M max steps (6502 executes ~30 M instructions real). **This test found two real bugs on first integration**: `PHP` wasn't forcing bits 4+5 in the pushed status byte, and `BRK` pushed PC+1 instead of PC+2 (missing the signature-byte skip). Any future CPU refactor — notably the planned `switch`-based dispatch — is gated by this test.

**`peripheral_bus_smoke`** — 5 assertions against `PeripheralBus` with fake lambdas, no `Memory`, no peripherals (self-contained, < 10 ms). Pins: `pageMask` fast-path miss at unrelated addresses, basic read routing, priority ordering at SID ↔ TMS9918 overlap, `setEnabled(false)` → fall-through / `setEnabled(true)` → re-activation, sniffer pass-through (`onWrite = {}` → `tryWrite` returns `false` so the byte lands in RAM). See `tests/peripheral_bus_smoke_test.cpp` for the pattern to follow when adding more single-threaded invariant tests — `<cassert>` + `add_test` is enough for now; GTest/Catch2 only earn their keep when the SID ring buffer test (multi-threaded) lands.
