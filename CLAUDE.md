...
- `$D012` (DSP) — write with **bit 7 set** triggers display callback + busy-counter bump (real 74LS164 only latches on PB7=1 strobe; gates out WozMon's reset-time `LDY #$7F / STY $D012` that would otherwise paint a spurious `_` before the `\` prompt). TerminalCard + PR-40 sniffers receive every `$D012` write unfiltered — they apply their own bit-7 / printable-range logic downstream. Read returns bit 7 = 0 (ready) after terminal-speed delay, else bit 7 = 1 (busy); counter clamped ≥ 0. PR-40 busy OR-merged on read when switch mode ≠ Off.
