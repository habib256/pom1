...
- **CpuClock.h** — `POM1_CPU_CLOCK_HZ = 1 022 727` (14.31818 MHz ÷ 14). Single source of truth: the pacer's rate is `pom1CyclesPerSecond(cpf)`, x1 = the crystal exactly — never `cpf × 60` (x1's cpf counts a 59.94 Hz frame; that product ran POM1 0.1 % fast). `BeamClock.h` is the shared cycle→(line, tick) clock both video engines now use (`lineTickAt` is defined during blanking; `beamPosAt`'s `x` is not — sorting beam events by `x` reorders the VBlank events that set up the next frame).
...
- **The GEN2 window draws the NEXT field, not the latest** — `SnapshotPublisher` keeps a ring of completed fields (`Gen2FieldRing.h`: HGR latch, text pages, journal) and `Gen2FieldPacer` shows one per refresh, two in reserve. Drawing the latest made a one-line-per-field scroll stand still and jump two lines under a few ms of jitter (Bernie's "staggering along").
...
- **EmulationController** — façade over CPU + Memory + emulation thread, across **4 TUs**: `EmulationController.cpp` (CPU thread, run/stop/reset, step, breakpoints, key injection, pokes, the slice/pacing loop), `_State.cpp` (memory images, snapshots, rewind, ROM re-loads), `_Machine.cpp` (fidelity + diagnostic knobs), `_Cards.cpp` (cassette/audio + per-card passthroughs). The split is pure code motion. Native = dedicated thread; WASM = `pumpEmulationMainThread()`. Slice cap 6000 cycles desktop, 50 000 WASM. **`ExecutionMode::Deterministic`** (cycle-bounded headless runs) keeps that thread parked for good: only `runCyclesSync` and its kin advance the CPU, and `runCyclesSync` delivers queued keys at each slice head — a cycle run on the thread there is an uncounted, host-timed head start (`deterministic_run_smoke`). **Input movies** (`InputMovie.h`) need every CPU cycle to go through `runCpuCounted()` and every key through `drainKeyboard()`: a new path calling `cpu->run()` or `keyboard.drainTo()` directly runs cycles the movie clock does not see, and replays diverge (`input_movie_smoke`).
...
- **Snapshot v7's `RUN` section** carries what a cycle-exact resume needs and no other section held — DRAM-refresh switch and phase, pending interrupt cycles, GEN2 noise state, terminal field phase, queued keys. Input movies replay from a snapshot; without it a replay drifts by a refresh stall (`input_movie_smoke` diverges when it is skipped).
...
- **Anything that is a decision rather than a draw call belongs OUTSIDE the UI.** The UI is in no test binary, which left POM1's largest layer uncovered — and that is exactly where the august-2026 bugs lived. Ten pure seams carry that logic today, each with its own test and **no ImGui and no GLFW**: `Apple1KeyMap`, `FullscreenExpand`, `WindowGeometry`, `LayoutDecisions.h`, `PresetDecisions.h`, `ShortcutTable.h`, `SoftwareDirRules.h`, `CommandPalette.h`, `TerminalTiming.h`, `FileBrowserDecisions.h`. **Grow this seam when adding UI logic.**
...
- **POM1's own file browser must be able to leave POM1's data directory.** It could not — `..` stopped at `softAsmRoot`, deliberately — and that root is wherever `ResourceLocator` found `software/`, which inside an AppImage is the read-only `/tmp/.mount_*` self-mount. So on any box whose native picker POM1 cannot use (WASM, a Linux desktop without zenity/kdialog, the Pi default) **no file of the user's own could be loaded at all** — reported by Uncle Bernie for his GEN2 demo work. The decisions are pure in `src/FileBrowserDecisions.h` (`file_browser_decisions_smoke`); two more halves of the same report: a **typed** path now sets the loader the way a **clicked** one always did (the field defaults to hex dump, so a typed `.bin` went to the WOZMON text parser and was refused — on a good file), and a native picker that **fails to launch** disarms itself so callers fall back to the built-in browser instead of File ▸ Load Memory doing nothing at all (a forked zenity inherits an AppImage's `LD_LIBRARY_PATH` and dies; pinned by `native_file_dialog_smoke` §5). `NativeFileDialog` stays free of POM1 services — the caller logs it. The **cassette** browser had the identical confinement and header bug and now calls the same two functions, which is why the root's NAME is a parameter of `displayDirectory` and not a literal.
- **Linux file pickers: portal → zenity → kdialog → POM1's own browser** (`NativeFileDialog.cpp`, protocol strings pure in `PortalFileChooser.h`). libdbus-1 is `dlopen`'d, never linked. A backend that cannot RUN is struck and the next serves the SAME request; **portal response 2 is a Cancel** (GTK sends it when the dialog's window is closed) — treat it as a failure and closing a dialog pops zenity. Every Load/Save caller falls back to the ImGui browser when `isAvailable()` goes false after a failed pick, and that browser shows `unavailableHint()`. `POM1_FILE_DIALOG=portal|zenity|kdialog|builtin` forces one; the tests rely on it.
- **Auto-enable by source dir** of a loaded file is the table `pom1::softwaredir::kRules` (`src/SoftwareDirRules.h`), read forwards by `performMemoryLoad` and backwards by the file picker. The match is on a path COMPONENT under either separator; FIRST rule wins. **No folder rule → the code decides** (`src/ProgramCardSniff.h`): absolute accesses to $C250-$C257 name the GEN2, whose canonical row (`ruleForCard`) then applies exactly as the folder would — a GEN2 program dropped from anywhere else ran on a card-less machine and spun forever on HST0. GEN2 only: the other cards' windows overlap.
...
- **Architecture trend guard** — `tools/check_architecture.py` (ctest `architecture_check`) rejects any new direct include from the core/device set toward ImGui or `MainWindow`, and reports ceilings. **`tools/architecture_baseline.json` is a ratchet, not a target architecture: lower a ceiling when a refactor improves it; never raise one merely to make CI green.** A raise must name what bought it, in `CHANGELOG.md` — see the entries for the audio seam, the lazy SID, the card-shadow warning, the dynamic preset table, `PriorityMutex::yieldToWaiters()` and the file-browser fix. Two sibling ratchets: the **devtools boundary** (17 edges, four `MainWindow_*` files — the list can only shrink) and the **frozen facades** (`memory_public_methods` 190, `controller_public_methods` 203 — counts, not lines, because a facade shrinks by losing methods).
