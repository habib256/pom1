...
| `--codetank-jumper <lower\|upper>` / `--codetank-rom <path>` | B | Pick 16 kB half of 32 kB 28c256 / override default `roms/codetank.rom`. |
...
- **CodeTank** — standalone P-LAB ROM card (split from JukeBox April 2026 so it can pair freely with TMS9918). Single 32 KB 28c256, jumper picks 16 KB half wired into fixed `$4000-$7FFF`. No `$CA00` latch, no paging. Read-only (real cards reflashed externally). On plug: `Memory::loadCodeTankRom` probes a hardcoded path then `codetank.rom` / `roms/codetank.rom`. **Hardware → CodeTank Library** scans `roms/codetank/*.{rom,bin}` (32 KB images + optional `.txt` sidecar). Bus priority 20, mutex only with Juke-Box — coexists with TMS9918, microSD, CFFA1. Preset 8 (P-LAB Apple-1 with TMS9918) plugs CodeTank by default. Pinned by `codetank_smoke`.
...
$4000-$7FFF  CodeTank ROM (16 KB half of 32 KB 28c256, mutex with Juke-Box)
