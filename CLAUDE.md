...
- **MainWindow_ImGui.cpp/h**: Main application window with menu bar, toolbar with icon buttons, status bar, window management, CPU speed control (1/2/Max MHz), file loading/saving dialogs, clipboard paste, memory map window, and keyboard input handling. **Machine Presets** (`Hardware > Machine Preset` submenu): 5 named configurations (`kMachinePresets[]` in anonymous namespace) each defining hardware card flags and a default ImGui window layout. Applied via `applyMachineConfig(int)` which sets all hardware flags atomically and populates `pendingLayout`; `applyPendingLayout(const char*)` is called before each hardware window `Begin()` to reposition with `ImGuiCond_Always`.
...
### v1.7.2 (April 2026) — Machine Presets & Windows Packaging

- **Machine Presets** (`Hardware > Machine Preset`): 5 named hardware configurations — **Woz Apple 1 (1976)**, **Replica 1 (Briel)**, **Bernie's Apple 1**, **P-LAB Apple 1**, **POM1 Full** — each enabling the right cards atomically and snapping windows into a default layout. `kMachinePresets[]` in anonymous namespace; `applyMachineConfig(int)` sets all hardware flags, `applyPendingLayout()` repositions windows with `ImGuiCond_Always` before each `Begin()`
- **Windows packaging**: `package_windows_release.bat` creates a self-contained release archive (DLLs, ROMs, software, fonts, sdcard, cassettes, docs) ready for distribution; `packaging/windows/LISEZ-MOI.txt` (French quick-start instructions); `CMakeLists.txt` updated for install rules; `run_emulator.bat` improved robustness; MSVC compatibility fixes in M6502.cpp/M6502.h, Memory.cpp/Memory.h, SID.cpp/SID.h, main_imgui.cpp
