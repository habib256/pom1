...
- **MainWindow_ImGui** — single class across **9 TUs** sharing helpers via `MainWindow_Internal.h` (`pom1::mainwindow::detail`): `MainWindow_ImGui` (ctor/dtor, render, CPU control), `_Layout`, `_Presets` (`kMachinePresets[]`; migration target for external `presets.json`), `_Menu`, `_Dialogs` (About/Thanks/Welcome/Hardware/Display — texture loaders `ensureAboutPhotoTexture`/`ensureApple50LogoTexture`/`ensureAppIconTexture`, released in `destroyPom1()`), `_HardwareWindows` (GraphicsCard / TMS9918 / WiFiModem / TerminalCard / A1IO_RTC / Juke-Box; TMS9918 GL texture 256×192 RGBA `GL_NEAREST`), `_FileDialogs` (auto-enable-by-directory), `_DebugWindows`, `_Keyboard`.
  ...
  **`applyMachineConfig(int)` invariants**:
  ...
  - First-time use of a preset (no saved `.size` sidecar): `computePresetLayoutExtent(cfg, fallback)` → `glfwSetWindowSize`. `glfwCreateWindow` hints 1274×801 to avoid a resize flash on boot. WASM mirrors this in `getWasmCanvasPixelSize`.
  - **Preset-switch floor** — every preset's first-time sizing also takes the max with POM1 Fantasy's extent (1206×807 = Screen 843×701 + right column 338 + chrome), computed from `kMachinePresets[kMachinePresetCount - 1]`, so switching presets never shrinks the OS window. Presets whose layout naturally demands more keep their extent-driven size. Keeping POM1 Fantasy last in the array is what pins this reference — same invariant as the default-picker and the banner.
