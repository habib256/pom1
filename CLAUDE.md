...
- **`basic_native_codegen` / `basic_native_run`** — the **native** BASIC→6502 compiler (`src/BasicNativeCompiler.*`, runtime `dev/lib/basicrt/`, [`doc/BASIC_COMPILER.md`](doc/BASIC_COMPILER.md)): integer **and** binary32-**float** phases. Codegen unit pins (strength reduction, native FOR/GOTO/GOSUB, graphics ABI, float ops via `fp_*`) + a cc65-gated end-to-end run that builds the standalone binary (loads/runs at `$0300`, no interpreter), runs it vs the interpreter, and asserts **same framebuffer + native faster** — ~22× integer compute, ~4.5× pixel-bound, ~2.0× float (FP-bound ceiling). `basic_native_run` skips (77) without cc65.
- **`basic_float_runtime`** — the standalone **binary32 software-float** runtime (`dev/lib/basicrt/basicrt_float.s`, the FP-phase foundation, no Applesoft ROM): cc65-gated, assembles the runtime and checks `fp_fromint16/toint16/add/sub/mul/div/cmp` against the host IEEE `float` over a grid + 4000 random pairs (2^±20). Skips (77) without cc65.
