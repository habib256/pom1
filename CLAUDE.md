...
| `--enable <card>[,…]` / `--disable <card>[,…]` | B | Rewrite deferred plug list. Names: `aci, sid, sid-se, microsd, tms9918, a1io-rtc, hgr, cffa1, krusader, wifi, terminal, jukebox, codetank, pr40, gt6144` (aliases in `kCardNames`). Mutual-exclusion (SID↔SID-SE, TMS9918↔SID-SE, JukeBox↔CodeTank, …) enforced at override. `--disable krusader` is a no-op (ROM unload needs hard reset). |
| `--sid-chip <6581\|8580>` | B | Swap A1-SID / SE chip model; libresidfp replays last register state. |
| `--jukebox-jumper <ram16\|ram32>` | B | Juke-Box: `RAM16_ROM32` → ROM `$4000-$BFFF`; `RAM32_ROM16` → ROM `$8000-$BFFF`. |
| `--jukebox-chip <flash\|eeprom>` | B | Physical chip mode: paged Flash (default, read-only) or 28c256 EEPROM (writable single page). Swap triggers a ROM reload. |
| `--codetank-jumper <lower\|upper>` | B | CodeTank board jumper — picks which 16 kB half of the 32 kB 28c256 is wired into `$4000-$7FFF`. |
| `--codetank-rom <path>` | B | Override the default `roms/codetank.rom` probe with a specific 32 kB ROM image (one CodeTank cartridge = two 16 kB programs back-to-back). Loaded at deferred plug. |
...
- **MainWindow_ImGui** — single class across **9 TUs** sharing helpers via `MainWindow_Internal.h` (`pom1::mainwindow::detail`): `MainWindow_ImGui` (ctor/dtor, render, CPU control), `_Layout`, `_Presets` (`kMachinePresets[]`; migration target for external `presets.json`), `_Menu`, `_Dialogs` (About/Thanks/Welcome/Hardware/Display — texture loaders `ensureAboutPhotoTexture`/`ensureApple50LogoTexture`/`ensureAppIconTexture`, released in `destroyPom1()`), `_HardwareWindows` (GraphicsCard / TMS9918 / WiFiModem / TerminalCard / A1IO_RTC / Juke-Box; TMS9918 GL texture 256×192 RGBA `GL_NEAREST`), `_FileDialogs` (auto-enable-by-directory), `_DebugWindows`, `_Keyboard`.
  ...
  **`applyMachineConfig(int)` invariants**:
  ...
  - `applyPendingLayout(const char*)` runs before each `Begin()` with **`ImGuiCond_FirstUseEver`**: preset ships defaults, user drags persist. **Widgets must not call `SetNextWindowSize(..., FirstUseEver)` themselves** — that runs after `applyPendingLayout` and overrides preset. `SetNextWindowSizeConstraints` is OK.
  - **Per-preset ini files** `ini/imgui_preset_NN.ini` + `ini/preset_NN.size`. `io.IniFilename = nullptr` disables ImGui's global `imgui.ini`; POM1 manages per-preset files via `savePresetLayout`/`loadPresetLayout`. The `.ini` holds window positions/sizes, the `.size` sidecar stores GLFW OS-window `W H` so preset switches restore the outer frame too. `applyMachineConfig(N)` saves the outgoing preset then loads the incoming one before setting `activePresetIndex = N`; clean shutdown saves again before `ImGui::DestroyContext()`. `ini/` is created at boot.
  - **Default preset = last entry** of `kMachinePresets[]` (*POM1 Multiplexing Fantasy (2026)*, index `kMachinePresetCount - 1` = 14). Shipped layout: Screen (10,61) 843×701 / Welcome (858,61) 338×223 / Cassette Deck (858,288) 338×476. The preset table (see `MainWindow_Presets.cpp` Preset 14) is authoritative. First-time use writes `ini/imgui_preset_14.ini` + `preset_14.size` and `glfwSetWindowSize` to `max(computePresetLayoutExtent(cfg, fallback), Fantasy extent)` — the Fantasy floor 1206×807 (Screen 843×701 + right column 338 + chrome) means switching presets never shrinks the OS window; presets demanding more keep their extent-driven size. `glfwCreateWindow` hints 1274×801 to avoid a resize flash; WASM mirrors this in `getWasmCanvasPixelSize`. **Keep POM1 Fantasy last when adding presets** — the default-picker, banner (`screen->setShowBanner(presetIndex == kMachinePresetCount - 1)`), and window-size floor all key off the terminal index.
  - Auto-enable by source dir of loaded file: `software/hgr/` → GEN2, `software/tms9918/` → TMS9918, `software/net/` | `/wifi/` → Wi-Fi Modem (`wifiModemReset()` on reload), `sdcard/` → microSD, `software/a1io_rtc/` → A1-IO & RTC, `software/gt-6144/` → GT-6144 (matches both cases). **`software/sid/` is intentionally NOT listed** — SID auto-plug desyncs UI flag vs Memory state across hardReset → card plugged-but-silent. SID programs rely on a preset with `sid=true`.
...
- **JukeBox** — P-LAB Apple-1 Juke-Box (Claudio Parmigiani & Jacopo Rosselli). Chip modes: **Flash** (paged read-only, 16 KB–512 KB; default) and **EEPROM 28c256** (32 KB single-page, writable). ROM window at `$4000-$BFFF` (RAM-16/ROM-32 jumper) or `$8000-$BFFF` (RAM-32/ROM-16), with Program Manager at `$BD00` and Save Program at `$B800`. **Bank-select latch at `$CA00`**: bits 0–3 = `Px` page, bit 4 = `Sx` sub-page. Bus handles: priority 20 for the active ROM window + priority 15 for `$CA00`; exactly one ROM handle enabled at a time. **Mutex with CodeTank, CFFA1, microSD, Krusader, Wi-Fi Modem, A1-SID** remains conservative. A1-AUDIO SE at `$CC00-$CC1F` coexists. Boot page is the lowest page with `$A5` at file offset `$7D00`. Invariants pinned by `jukebox_paged_rom_smoke`.
- **CodeTank** — Standalone P-LAB ROM card (formerly bundled inside JukeBox as a chip mode; split out April 2026 so it can pair freely with TMS9918). Single 32 KB 28c256 EEPROM with a board jumper that picks which 16 KB half is wired into the fixed `$4000-$7FFF` window. No `$CA00` latch, no Program Manager, no paging — just one contiguous 16 KB ROM. POM1 models the chip as read-only (real CodeTanks are reflashed externally). On plug, `Memory::loadCodeTankRom` probes `/home/gistarcade/Téléchargements/28c256_Final.bin` first, then `codetank.rom` / `roms/codetank.rom`. The Hardware menu offers a **CodeTank Library** window that scans `roms/codetank/*.{rom,bin}` (32 KB images, optional same-name `.txt` sidecar for description) and also lists the legacy `roms/codetank.rom`; clicking *Load Lower 16 kB* / *Load Upper 16 kB* loads the chosen ROM, sets the jumper, and plugs the card. Bus handle: `$4000-$7FFF` priority 20, mutex only with the Juke-Box (overlapping ROM window) — coexists with TMS9918, A1-AUDIO SE, microSD, CFFA1 etc. on the same bus. Designed to ship Apple-1 software targeting the P-LAB TMS9918 Graphic Card on real silicon — a TMS game can live on its own CodeTank ROM and run on a P-LAB stack without depending on the cassette deck or microSD. Invariants pinned by `codetank_smoke` (`tests/codetank_smoke_test.cpp`). CLI: `--enable codetank`, `--codetank-jumper lower|upper`, `--codetank-rom <path>`.
- Preset 8 **P-LAB Apple-1 with TMS9918 Graphic Card** intentionally also plugs CodeTank by default (`CodeTank::Jumper::Lower16`, default ROM probe) because CodeTank only occupies `$4000-$7FFF` and does not conflict with TMS9918 at `$CC00/$CC01`.
...
- **TerminalCard** — passive bridge: sniffs `$D012` writes (hook in `memWrite`), injects keys into `$D010`/`$D011`. TCP server on IPv4 loopback :6502 (IPv6 `::1` refused — `telnet localhost` falls back to `127.0.0.1`). Modes: 7-bit (CR→CRLF, optional uppercase via Ctrl-O/I) and 8-bit raw (Ctrl-T, fires in either mode). Controls: Ctrl-L clear, Ctrl-R reset; each has an **ESC-prefixed alternate** (ESC T/O/L/R/I) for tty line disciplines that eat Ctrl-T/O/R. **Ctrl-S / ESC S — Screenshot for LLM agents**: arms `screenshotPending`; the main render thread, between `ImGui_ImplOpenGL3_RenderDrawData()` and `glfwSwapBuffers()` in `main_imgui.cpp`, runs `glReadPixels` on the full back-buffer (every visible window — Apple 1 screen + GraphicsCard + TMS9918 + GT6144 + dialogs all at once), Y-flips, and writes `screenshots/pom1_latest.png` via stb_image_write. The absolute path is queued back via `setScreenshotResult()` and emitted on the next `advanceCycles` poll as `\r\n[SCREENSHOT: /abs/path]\r\n`. Result string uses its own `screenshotResultMutex` so the render thread never blocks on the long-held `cardMutex`. Unknown ESC sequences forward the ESC so ANSI still reaches the Apple 1. On accept: `IAC WILL ECHO` + `IAC WILL SUPPRESS-GO-AHEAD` + `IAC DO SUPPRESS-GO-AHEAD` flips the client to character-at-a-time. Pending reset/clear use `std::atomic<bool>` consumed **outside** `stateMutex` to avoid deadlock with `runEmulationSlice()`. Desktop only.
...
- **GT6144** — SWTPC GT-6144 Graphic Terminal (1976, $98.50). 64×96 mono framebuffer on 6× Intel 2102 SRAM, **write-only** I/O at `$D00A` (PIA `A3` chip-select). 4-phase FSM on a single port: `0..63` latch X + pixel OFF; `64..127` latch X + pixel ON; `128..223` commit Y (uses latched X + state); `224..255` control opcode (`byte & 0x07`: 0=INVERTED / 1=NORMAL / 4=UNBLANK / 5=BLANK / 2,3=CT-1024 mix (no-op standalone) / 6=reserved / 7=NORMAL alias; bits 3-4 are don't-cares so `224/232/240/248` all mean INVERTED). Inversion and blanking live in the render path — framebuffer untouched (matches analog XOR at video output). Bus `{0xD00A,0xD00A}` priority 0 suffices: `memWrite` dispatches the bus **before** `$D0xx` PIA aliasing; reads fall through to PIA alias (no read-back on real hardware). Power-on framebuffer seeded with `std::random_device` → `mt19937` to reproduce the Intel 2102 bistable "rectangles aléatoires"; reseeds on every `setGT6144Enabled(true)`. No bus overlap with other POM1 peripherals — composes freely. **Display aspect** — the 64×96 matrix (logically 2:3) fed a 4:3 CRT, so SWTPC's manual describes visible pixels as "petits rectangles" (2:1 W:H). `renderGT6144Window` uploads at native 64×96 but passes `displayAspectW = kWidth * 2` to `layoutFitVideoViewport`, letting GL_NEAREST do the 2× horizontal blit-time stretch. Any rewrite back to square pixels is a bug.
- **PR40Printer** — SWTPC PR-40 Alphanumeric Printer (1976), per Jobs' *Interface Age* Oct-76 mod. **No MMIO** — third sniffer on `$D012` writes (after `DisplayDevice::onChar` + `TerminalCard::onDisplayWrite`). 40-char FIFO flushed on CR (`$0D`) or full; flush arms `kMechCycleCpu = POM1_CPU_CLOCK_HZ * 4 / 5` (~818 182 cycles ≈ 0.8 s) countdown, decremented from `Memory::advanceCycles()`. DPDT switch modes (selectable in Hardware window): **Off** = disconnected; **Mixed** = Jobs' 2-pos wiring, PB7 reflects video-busy OR printer-busy (CPU stalls for either); **PrintOnly** = community 3-pos mod, PB7 = printer-busy alone (CPU can flood the FIFO at 1 MHz). Busy-OR merge lives inline in `Memory::memRead($D012)`. Paper roll is unbounded `std::vector<std::string>` + in-progress `currentLine`; UI uses `ImGuiListClipper` to stay O(visible). `pr40_printer_smoke` pins the wiring.
...
- `$D012` (DSP) — write triggers display callback + busy-counter bump for every value except **raw `$7F`** (WOZ Monitor's reset-time `LDY #$7F / STY $D012` DDR setup, which would otherwise paint a spurious `_` before the `\` prompt). Real hardware only latches on PB7=1; POM1 is historically permissive so emulator-era demos (e.g. `software/tms9918/demo.bin`) that call WOZ ECHO with bit 7 clear still render — keep this invariant. `$DF` via ECHO still yields `_` (`value & 0x7F = $5F`, the underscore glyph), so the filter only drops the DDR write, not legitimate underscores. TerminalCard + PR-40 sniffers get every write unfiltered — they apply their own bit-7 / printable-range logic downstream. Read: bit 7 = 0 (ready) after terminal-speed delay, else bit 7 = 1 (busy); counter clamped ≥ 0. PR-40 busy OR-merged on read when switch mode ≠ Off.
...
$4000-$7FFF  CodeTank ROM window (selected 16 KB half of 28c256, fixed decode; standalone card, mutex with Juke-Box)
...
- **macOS** — links Cocoa + IOKit + CoreVideo. `GLFW_OPENGL_FORWARD_COMPAT` macOS-only. Window icon is a no-op (OS pulls from `.app` bundle). **Packaged data-path split**: `package_macos_release.sh` places read-only assets (`roms/`, `fonts/`, `software/`, `pic/`, `cassettes/`, and `sdcard/` / `cfcard/` seeds) in `POM1.app/Contents/Resources/`. At startup, `pom1_macos_provision_user_data_dir()` creates `~/Library/Application Support/POM1/` with symlinks back into the bundle for read-only dirs + seeded writable dirs for `sdcard/` / `cfcard/` / `ini/`, then chdirs there. Symlinks refresh each launch so Gatekeeper App Translocation and `/Applications` drag-installs both work. Dev flow (`build/POM1` symlink) falls back to bundle-parent chdir when `Contents/Resources/roms` is absent. DMG carries `.VolumeIcon.icns` + `SetFile -a C` for the Finder volume icon.
...
- **`codetank_smoke`** — self-contained (`CodeTank` + `Logger`). Loads `roms/codetank.rom`, pins the 32 kB exact-size requirement, the lower/upper jumper offset math (`$4000` ↦ file `$0000` or `$4000`), the read-only invariant (`writeByte` is a no-op), the previous-contents-preserved-on-rejection contract, and the Snapshot round-trip (jumper / path / loaded flag).
