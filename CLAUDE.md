...
- **GraphicsCard** — Uncle Bernie's GEN2 HGR. Reads RAM `$2000-$3FFF`, renders 280×192 NTSC artifact colour. Apple II non-linear scanlines. `rasterizeLine` is a 14 KB LUT `kHgrPixels[(colParity << 8) | byte]` (no inter-byte neighbours), memcpy-copied; second pass paints white at 39 inter-byte seams when bit 6 ↔ bit 0 both lit. 9th index bit is `colParity` (absolute screenX parity), **not** group2. **UI-owned** (`MainWindow_ImGui::graphicsCard`), not a `Memory` peripheral — it's a passive end-of-frame rasteriser fed the full 64 KB at render time.
- **Gen2VideoScanner** — cycle-accurate video *address* generator for the GEN2 **release** card (Phase 1 of the beam-racing back-port POM2→POM1; see `TODO.md`). Unlike `GraphicsCard`, this **is** `Memory`-owned (value member `gen2Scanner`), advanced from `Memory::advanceCycles()` when `hgrFramebufferAttached` (gated like TMS9918/SID; phase reset on cold plug). Maintains `cycleCounter % (65×262)` — NTSC Apple II timing verbatim POM2 (`kCyclesPerLine=65`, `kLinesPerFrame=262`, `kCyclesPerFrame=17030`). `scannerAddress()` is a verbatim port of POM2 `Memory::floatingBus()` (MAME `apple2video.cpp scanner_address`), stripped of the IIe inputs (80STORE, iieMode) but **keeping the II/II+ text-mode HBL phantom row `$1000`**; `floatingBus(mem)` reads the sampled byte (HGR page 1 `$2000`, page 2 `$4000`). Exposed via `Memory::peekGen2VideoCycle()` / `gen2FloatingBus()` / `setGen2DisplayState()`. **Foundation only** — rendering still goes through the passive `GraphicsCard`; Phase 2 wires the `$C250-$C257` soft switches + MSB blank (HST0) flag, Phase 3 the beam-raced render. **Bernie's full release-card spec is confirmed** (`doc/ColorGraphicsCard_doc_for_Arnaud.pdf`, transcribed in `doc/GEN2_RELEASE_questions.md`): soft switches are read-only (read toggles + returns HST0 in D7, writes no-op), HST0 = 1 during H/V-blank with a 0 notch in the 3-cycle color burst, page-2 HIRES at `$4000-$5FFF` (`$C255`), decode `SEL = $Cxxx & !A11 & A9 & A4` (mirrors across `$C2/$C3/$C6/$C7xx`), 65 cyc/line × 262 (60 Hz) / 312 (50 Hz). Pinned by `gen2_floatingbus_smoke`.
...
- **CodeTank** — P-LAB ROM **daughterboard** of the TMS9918 Graphic Card (split from JukeBox April 2026). No standalone edge connector / address decoder on the Apple 1 bus. `Memory::setCodeTankEnabled(true)` auto-plugs TMS9918; `setTMS9918Enabled(false)` cascade-unplugs CodeTank. Single 32 KB 28c256, jumper → 16 KB half at `$4000-$7FFF`; no `$CA00`, read-only. ROM probe order: `roms/codetank/Codetank_GAME1.rom`, then `codetank.rom` / `roms/codetank.rom`.
  ...
  - **UI / bus**: **File → P-LAB CodeTank Library** scans `roms/codetank/*.{rom,bin}`; priority 20; mutex with Juke-Box; always with TMS9918. Preset 8 plugs both. Tests: `codetank_smoke`, `codetank_tms9918_dependency`, `codetank_game4_smoke`.
