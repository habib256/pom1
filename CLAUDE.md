...
- **`aci_tape_loading`** — full pulse-extraction → ACI ROM → RAM pipeline on `cassettes/APPLE50TH.ogg`; asserts the demo's first three bytes (`A9 FF 48` = `LDA #$FF / PHA`) land at `$0280` (the tape's natural load address — see `cassettes/tapeinfo.txt`). Gate for any change to pulse extraction, ACI ROM emulation, or keyboard-input wiring. Also validates `kTapeFileTimebaseHz = POM1_CPU_CLOCK_HZ`. Needs `${CMAKE_SOURCE_DIR}` cwd so `roms/` + the `.ogg` resolve.
