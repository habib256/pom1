...
- **HGR Paint editor** (`src/Pom1HgrPaintHost.{h,cpp}` here, portable `hgrpaint/HgrPaintEditor.{h,cpp}` + `HgrPaintModel.{h,cpp}` + `IHgrPaintHost.h` at repo root — same split as Bench, intended for POM2 reuse). The editor + model depend only on ImGui/GL and the `hgrpaint::IHgrPaintHost` seam (poke / `renderHgrPage` / file I/O); POM1's host wraps `GraphicsCard` + `EmulationController`. The pure model owns its own Apple II HGR interleave + geometry constants (no `GraphicsCard` dependency). **Flood fill floods by *perceived* artifact colour** (renders the page through the host NTSC pipeline): a chromatic field is bit-dithered (solid violet = `$55`, odd columns off), so a raw-bit flood leaks through it — `fillRegion` clears the region then stamps the new colour's parity pattern. Pinned by `hgr_paint_plot_smoke` (now also links `hgrpaint/HgrPaintModel.cpp`).
