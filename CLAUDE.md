...
**cc65:** per-project Makefiles call `ca65`+`ld65`, then `python3 emit_*_txt.py` lands `.bin`+Woz-hex `.txt` under `software/<dir>/`. `make -C dev/projects` is the CI gate. Linker configs in `dev/cc65/`. Hot paths in the gen2c runtime are hand-written 6502 in `gen2_blit.s` — anything compiling `gen2.c` must also assemble it.
