...
- **NativeFileDialog** (`src/NativeFileDialog.{h,cpp}` + `_Mac.mm`) — opens the OS's localised file picker for Load/Save Memory, Load/Save Tape, Load/Save Snapshot. Win32 `GetOpenFileNameW`/`GetSaveFileNameW` (links `comdlg32`), Cocoa `NSOpenPanel`/`NSSavePanel`, Linux probes `$PATH` for `zenity` then `kdialog` (forked, stdout-captured) on first call and caches the choice. WASM and Linux-without-zenity-or-kdialog fall back to the existing ImGui browser; Load Memory binary still pops a small ImGui "address only" follow-up since `.bin` files have no embedded address. Cassette deck's Load/Save buttons route through the same action methods so they pick up native too.
