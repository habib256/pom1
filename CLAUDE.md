...
- **`basic_native_codegen` / `basic_native_run`** — the **native** BASIC→6502 compiler (`src/BasicNativeCompiler.*`, runtime `dev/lib/basicrt/`, [`doc/BASIC_COMPILER.md`](doc/BASIC_COMPILER.md)): codegen unit pins (strength reduction, native FOR/GOTO/GOSUB, graphics ABI) + a cc65-gated end-to-end run that builds the standalone binary (loads/runs at `$0300`, no interpreter), runs it vs the interpreter, and asserts **identical framebuffer + native faster** (~25× on arith-bound code). `basic_native_run` skips (77) without cc65.
