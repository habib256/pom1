...
Result compile(const std::string& source, Card card)
{
    ...
    // sees them used below -> short/fast zp addressing (a forward-declared label
    // would fall back to absolute). Variables + temporaries + FOR limit/step slots.
    o << ".segment \"ZEROPAGE\"\n";
    ...
    o << "\n.segment \"CODE\"\n";
    o << "basic_main:\n";
    // Variables default to 0 (temporaries + FOR slots are always written before
    // read, so they need no init). Variables live in zero page now, so this is a
    // few cheap stores rather than a page-clear loop.
    o << "\tlda #0\n";
    for (const std::string& v : g.vars)
        if (!v.empty() && v[0] != '\0')
            o << "\tsta V_" << v << "\n\tsta V_" << v << "+1\n";
