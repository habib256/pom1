...
// Total NMOS cycle counts for undocumented opcodes dispatched through Unoff1/2/3
// (0 = documented opcode — never consulted). Values follow masswerk.at's illegal-
// opcode tables. Two families in matrix columns 3/7/F need care because they are
// NOT the read-modify-write ops that share those columns:
//   - rows $8x SAX (store):  $83=6 $87=3 $8F=4 $97=4  (vs RMW 8/5/6/6)
//   - rows $9x SHA (store):  $93=6 $9F=5              (vs RMW 8/7)
//   - rows $Ax/$Bx LAX/LAS (load): $A3=6 $A7=3 $AF=4 $B3=5 $B7=4 $BB=4 $BF=4
// The indexed-absolute reads add +1 on a page cross in Unoff3: the abs,X NOPs
// (1C/3C/5C/7C/DC/FC) and the abs,Y loads LAS ($BB) / LAX ($BF). (The (zp),Y
// load LAX $B3 also crosses on real silicon; that +1 is not modelled here.)
constexpr uint8_t kIllegalTotalCycles[256] = {
...
// Illegal opcodes routed through Unoff3 that perform an indexed-absolute read and
// take the +1 page-cross penalty. abs,X NOPs (1C/3C/5C/7C/DC/FC) index by X; the
// abs,Y loads LAS ($BB) / LAX ($BF) index by Y. Returns false for the plain
// 3-byte illegals (RMW/SAX/SHA) whose cycle total is fixed regardless of cross.
bool isAbsIndexedIllegalRead(uint8_t opcode, bool& indexByY)
{
    switch (opcode) {
    case 0x1C: case 0x3C: case 0x5C: case 0x7C: case 0xDC: case 0xFC:
        indexByY = false; return true;
    case 0xBB: case 0xBF:
        indexByY = true; return true;
    default:
        return false;
    }
}
...
{
    cycles = kIllegalTotalCycles[lastFetchedOpcode];
}
