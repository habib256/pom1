        ...
        while (i < src.size() && src[i] != '\n') {
            ...
            if (std::string("+-*/^=<>").find(c) != std::string::npos) {
                ...
                if (i + 1 < src.size()) {
                    ...
                    // Applesoft relational pairs: <=,<> (and the < forms),
                    // >=,>< , and the =<,=> spellings. Anything else (e.g. ">>")
                    // stays a single-char op so prec() rejects it cleanly.
                    bool pair = (c == '<' && (n == '=' || n == '>')) ||
                                (c == '>' && (n == '=' || n == '<')) ||
                                (c == '=' && (n == '<' || n == '>'));
                    if (pair) { op += n; ++i; }
                }
                // canonicalise the alternate Applesoft spellings
                if (op == "><") op = "<>";
                else if (op == "=<") op = "<=";
                else if (op == "=>") op = ">=";
...
{
    const size_t startTok = p;
    if (!unary(d)) return false;
    // Strength-reduce "K * <expr>" only when the left operand was a single bare
    // literal token. lastNum is a shared member that nested primary() calls
    // mutate, so after a parenthesised / unary-minus / function sub-expression
    // (e.g. "(B+1)", "-2", "ABS(N)") it can hold stale state leaked from the
    // last literal seen *inside* that sub-expression. Trusting it there would
    // take the K*<expr> branch and silently drop the real left operand. The
    // token-count check pins it to a genuine top-level literal.
    bool leftNum = lastNum && (p == startTok + 1); long leftVal = lastNumVal;
    for (;;) {
        std::string op;
...
Result compile(const std::string& source, Card card, FpMode mode)
{
    ...
    // GEN2 HPLOT reads `pen` (zero-page), which is otherwise seeded only by rt_hgr.
    ...
    // Seed GEN2 pen so HPLOT before HGR/HCOLOR draws instead of erasing.
    if (seedPen) o << "\tlda #3\n\tsta pen\n";
    o << body;
    o << "basic_done:\n\tjmp basic_done\n\n";
